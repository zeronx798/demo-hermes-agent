...
const isDarkType = (raw, background) => {
 ...
 */
function extractTerminalPalette(colors, background) {
    const hex = (key) => normalizeHex(typeof colors[key] === 'string' ? colors[key] : null, background) ?? undefined;
    const palette = {};
...
const pick = (colors, keys, backdrop) => {
    for (const key of keys) {
        const value = normalizeHex(typeof colors[key] === 'string' ? colors[key] : null, backdrop);
        if (value) {
            return { key, value };
        }
    }
...
export function convertVscodeColorTheme(raw, opts = {}) {
    ...
    const card = take(['sideBarSectionHeader.background', 'tab.inactiveBackground', 'editorGroupHeader.tabsBackground'], mix(background, foreground, dark ? 0.04 : 0.025));
    ...
    const border = take(['panel.border', 'editorGroup.border', 'sideBar.border', 'contrastBorder', 'widget.border', 'input.border'], mix(background, foreground, dark ? 0.16 : 0.14));
    const input = take(['input.background', 'dropdown.background', 'quickInput.background'], mix(background, foreground, dark ? 0.1 : 0.06));
    const mutedForeground = take(['descriptionForeground', 'editorLineNumber.foreground', 'tab.inactiveForeground', 'disabledForeground'], mix(foreground, background, 0.45));
