...
const themeJson = (type, background, foreground) => JSON.stringify({ type, colors: { 'editor.background': background, 'editor.foreground': foreground } });
// A full base-8 ANSI set keyed off `red` so each variant is distinguishable.
const ansiColors = (red) => ({
...
const themeJsonWithAnsi = (type, background, foreground, red) => JSON.stringify({
    type,
...
describe('buildThemeFromMarketplace', () => {
    ...
    it('throws when the extension contributes no themes', () => {
        expect(() => buildThemeFromMarketplace({ extensionId: 'x.y', displayName: 'X', themes: [] })).toThrow(/does not contribute/i);
    });
});
