...
export function toggleReviewTreeMode() {
...
export const $reviewMaxChurn = computed($reviewFiles, files => files.reduce((max, file) => Math.max(max, file.added + file.removed), 0));
...
const repoCwd = () => $currentCwd.get()?.trim() || null;
...
export async function selectReviewFile(file) {
...
export function clearReviewSelection() {
    $reviewSelectedPath.set(null);
    $reviewDiff.set(null);
    $reviewDiffLoading.set(false);
}
// ── View state ───────────────────────────────────────────────────────────────
export async function refreshShipInfo() {
    const ctx = reviewCtx();
...
function refreshShipInfoIfStale() {
...
export async function stageReviewFile(path) {
    await desktopGit()?.review?.stage(repoCwd() ?? '', path);
    await afterMutation();
}
export async function unstageReviewFile(path) {
    await desktopGit()?.review?.unstage(repoCwd() ?? '', path);
    await afterMutation();
}
export async function revertReviewFile(path) {
...
export const $reviewRevertTarget = atom(undefined);
/** Open the revert confirm for a single file, or `null` for all changes. */
export function requestRevert(path) {
    $reviewRevertTarget.set({ path });
}
export function cancelRevert() {
...
export async function commitChanges(message, opts = {}) {
    const ctx = reviewCtx();
    if (!ctx || !message.trim()) {
        return;
    }
...
export function cancelCommitMessage() {
    commitGenSeq += 1;
...
export async function generateCommitMessage(previous = '') {
    const ctx = reviewCtx();
...
export async function createOrOpenPr() {
    const ctx = reviewCtx();
    if (!ctx) {
        return;
    }
