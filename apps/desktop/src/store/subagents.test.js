...
import { $subagentsBySession, activeSubagentCount, allSubagents, buildSubagentTree, clearSessionSubagents, failedSubagentCount, pruneDelegateFallbackSubagents, upsertSubagent } from './subagents';
const listFor = (sid) => $subagentsBySession.get()[sid] ?? [];
describe('subagent store', () => {
    ...
    it('captures live thinking/progress/tool stream lines', () => {
        upsertSubagent('s1', { goal: 'scan files', status: 'queued', subagent_id: 'a1', task_index: 0 }, true, 'subagent.spawn_requested');
        upsertSubagent('s1', {
            status: 'running',
            subagent_id: 'a1',
            task_index: 0,
        ...
        upsertSubagent('s1', { status: 'running', subagent_id: 'a1', task_index: 0, text: 'plan the search order' }, false, 'subagent.thinking');
        upsertSubagent('s1', { status: 'running', subagent_id: 'a1', task_index: 0, text: 'found candidate matches' }, false, 'subagent.progress');
        upsertSubagent('s1', { status: 'completed', subagent_id: 'a1', summary: 'search complete', task_index: 0 }, false, 'subagent.complete');
        const item = listFor('s1')[0];
