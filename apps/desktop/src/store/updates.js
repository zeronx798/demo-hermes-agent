...
export const $desktopVersion = atom(null);
export const $updateApply = atom(IDLE);
export const $updateChecking = atom(false);
...
export const setUpdateOverlayOpen = (open) => $updateOverlayOpen.set(open);
export const openUpdateOverlayFor = (target) => {
 ...
 */
export function reportBackendContract(contract) {
...
export function reportInstallMethodWarning(message) {
    if (!message) {
 ...
 */
export function maybeNotifyUpdateAvailable(status) {
 ...
 */
export function startActiveUpdate() {
    const target = isRemoteMode() ? 'backend' : 'client';
    $updateOverlayTarget.set(target);
    $updateOverlayOpen.set(true);
...
export async function refreshDesktopVersion() {
...
export async function applyUpdates(opts = {}) {
    const bridge = window.hermesDesktop?.updates;
    if (!bridge) {
...
async function waitForBackendReturn() {
...
function finishBackendApply(returned) {
    if (returned) {
        $backendUpdateApply.set(IDLE);
        setUpdateOverlayOpen(false);
...
function ingestBackendActionStatus(status) {
...
function ingestProgress(payload) {
    ...
    const terminal = payload.stage === 'error' ||
...
export function stopUpdatePoller() {
