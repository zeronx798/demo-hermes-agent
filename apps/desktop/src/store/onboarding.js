...
import { cancelOAuthSession, getGlobalModelOptions, getRecommendedDefaultModel, listOAuthProviders, pollOAuthSession, setEnvVar, setModelAssignment, startOAuthLogin, submitOAuthCode, validateProviderCredential } from '@/hermes';
...
function writeCachedConfigured(value) {
    if (typeof window === 'undefined') {
        return;
    }
    try {
...
function writeCachedSkipped(value) {
    if (typeof window === 'undefined') {
        return;
    }
    try {
...
let pollTimer = null;
let providersRefreshPromise = null;
const errMessage = (e) => (e instanceof Error ? e.message : String(e));
const patch = (update) => $desktopOnboarding.set({ ...$desktopOnboarding.get(), ...update });
const setFlow = (flow) => patch(flow.status === 'idle' ? { flow } : { flow, reason: null });
const sessionIdFor = (flow) => ('start' in flow && flow.start ? flow.start.session_id : undefined);
function clearPoll() {
    if (pollTimer !== null) {
...
async function checkRuntime(ctx, requestedProvider) {
...
function shouldPreserveConfiguredOnFallback(runtime, state) {
...
function notifyGatewayTools(tools) {
...
async function fetchProviderDefaultModel(preferredSlugs) {
    let options;
    try {
    ...
    const matched = providers.find((p) => lower.includes(String(p.slug).toLowerCase())) ?? providers[0];
...
async function completeWithModelConfirm(ctx, providerLabel, preferredSlugs, onFail, 
...
export function startManualOnboarding(reason = DEFAULT_MANUAL_ONBOARDING_REASON) {
    patch({
        manual: true,
        requested: true,
        localEndpoint: false,
...
export function startManualLocalEndpoint(reason = null) {
...
let pendingProviderOAuthId = null;
export function startManualProviderOAuth(providerId, reason = null) {
...
export function peekPendingProviderOAuth() {
...
export function setOnboardingMode(mode) {
    patch({ mode });
}
export async function refreshOnboarding(ctx) {
...
export async function startProviderOAuth(provider, ctx) {
    clearPoll();
...
async function pollSession(provider, start, ctx) {
    try {
        const { error_message, status } = await pollOAuthSession(provider.id, start.session_id);
        if (status === 'approved') {
            clearPoll();
            setFlow({ status: 'success', provider });
            await completeWithModelConfirm(ctx, provider.name, [provider.id], reason => setFlow({
                status: 'error',
                provider,
...
export function setOnboardingCode(code) {
    const { flow } = $desktopOnboarding.get();
    if (flow.status === 'awaiting_user') {
        setFlow({ ...flow, code });
    }
}
export async function submitOnboardingCode(ctx) {
    const { flow } = $desktopOnboarding.get();
...
async function copyAndFlash(text, predicate) {
    try {
...
export async function recheckExternalSignin(ctx) {
    const { flow } = $desktopOnboarding.get();
    if (flow.status !== 'external_pending') {
        return;
    }
...
export async function saveOnboardingLocalEndpoint(baseUrl, apiKey, ctx) {
...
export async function setOnboardingModel(model) {
    const { flow } = $desktopOnboarding.get();
...
export function confirmOnboardingModel(ctx) {
    const { flow } = $desktopOnboarding.get();
    if (flow.status !== 'confirming_model') {
        return;
    }
