...
import { collapseModelFamilies, defaultVisibleKeys, effectiveVisibleKeys, emptyProviderSentinelKey, isProviderSentinel, modelVisibilityKey, resolveVisibleKeys, toggleModelVisibility } from './model-visibility';
const provider = (slug, models) => ({
    models,
    name: slug,
    slug
});
...
describe('toggleModelVisibility', () => {
    ...
    const apply = (stored, slug, model) => toggleModelVisibility(stored, providers, slug, model);
    ...
    it('clears only the toggled provider sentinel when a model is re-enabled', () => {
        let stored = new Set([emptyProviderSentinelKey('openai'), emptyProviderSentinelKey('nous')]);
        stored = apply(stored, 'openai', 'gpt-a');
    ...
    it('re-enabling one model of a hidden-all provider restores ONLY that model, not the curated defaults', () => {
        ...
        let stored = new Set([emptyProviderSentinelKey('openai')]);
        stored = apply(stored, 'openai', 'gpt-a');
        const visible = effectiveVisibleKeys(stored, providers);
        expect(visible.has(modelVisibilityKey('openai', 'gpt-a'))).toBe(true);
