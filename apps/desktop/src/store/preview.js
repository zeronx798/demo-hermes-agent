...
export function setPreviewTarget(target) {
...
export function filePreviewTabId(target) {
    return `file:${target.url}`;
}
function openFilePreviewTarget(target) {
...
function isPreviewTarget(value) {
    ...
    const r = value;
    return ((r.kind === 'file' || r.kind === 'url') &&
        typeof r.label === 'string' &&
        typeof r.source === 'string' &&
        typeof r.url === 'string');
}
function isFilePreviewTab(value) {
    if (!value || typeof value !== 'object') {
        return false;
    }
    const r = value;
...
function isPreviewRecord(value) {
    if (!value || typeof value !== 'object') {
        return false;
    }
    const r = value;
    return (typeof r.createdAt === 'number' &&
        typeof r.id === 'string' &&
...
function loadSessionPreviewRegistry() {
...
function persistSessionPreviewRegistry(registry) {
    if (typeof window === 'undefined') {
        return;
    }
    try {
...
function pruneRegistry(registry) {
    const entries = Object.entries(registry)
        .map(([sessionId, records]) => [sessionId, [...records].sort((a, b) => b.createdAt - a.createdAt).slice(0, MAX_RECORDS_PER_SESSION)])
        .filter(([, records]) => records.length > 0)
        .sort(([, a], [, b]) => (b[0]?.createdAt ?? 0) - (a[0]?.createdAt ?? 0))
        .slice(0, MAX_SESSIONS);
    return Object.fromEntries(entries);
}
function currentPreviewSessionId() {
    return $selectedStoredSessionId.get() || $activeSessionId.get() || '';
}
function recordId(sessionId, target) {
    return `${sessionId}:${target.url}`;
}
export function registerSessionPreview(sessionId, target, source, rawTarget = target.source) {
    ...
    $sessionPreviewRegistry.set(pruneRegistry({
        ...current,
        [id]: [nextRecord]
    }));
    return nextRecord;
}
export function setSessionPreviewTarget(sessionId, target, source, rawTarget = target.source) {
...
export function setCurrentSessionPreviewTarget(target, source, rawTarget = target.source) {
    return setSessionPreviewTarget(currentPreviewSessionId(), target, source, rawTarget);
}
export function getSessionPreviewRecord(sessionId) {
    const id = sessionId?.trim();
    if (!id) {
        return null;
    }
...
export function dismissSessionPreview(sessionId, url) {
    const id = sessionId?.trim();
    if (!id) {
        return;
    }
...
export function closeOtherRightRailTabs(keepId) {
...
export function closeRightRailTabsToRight(tabId) {
...
export function beginPreviewServerRestart(taskId, url) {
    $previewServerRestart.set({ status: 'running', taskId, url });
}
export function completePreviewServerRestart(taskId, text) {
...
export function progressPreviewServerRestart(taskId, text) {
    const current = $previewServerRestart.get();
...
export function failPreviewServerRestart(taskId, message) {
    const current = $previewServerRestart.get();
    if (current?.taskId !== taskId || current.status !== 'running') {
        return;
    }
