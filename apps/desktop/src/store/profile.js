...
import { arraysEqual, persistBoolean, persistStringArray, persistStringRecord, storedBoolean, storedStringArray, storedStringRecord } from '@/lib/storage';
...
export function normalizeProfileKey(name) {
...
export const $activeProfile = atom('default');
...
export const $profileOrder = atom(storedStringArray(PROFILE_ORDER_STORAGE_KEY));
$profileOrder.subscribe(value => persistStringArray(PROFILE_ORDER_STORAGE_KEY, [...value]));
export function setProfileOrder(names) {
...
export function sortByProfileOrder(items, order) {
...
export const $profileColors = atom(storedStringRecord(PROFILE_COLORS_STORAGE_KEY));
...
export function setProfileColor(name, color) {
...
export function requestFreshSession() {
...
async function syncConnectionToActiveProfile(profile) {
...
export async function ensureGatewayProfile(profile) {
...
export const $showAllProfiles = atom(storedBoolean(SHOW_ALL_PROFILES_STORAGE_KEY, false));
...
// in sync.
export const $profileScope = computed([$showAllProfiles, $activeGatewayProfile], (showAll, gateway) => showAll ? ALL_PROFILES : normalizeProfileKey(gateway));
...
export function newSessionInProfile(name) {
    const target = normalizeProfileKey(name);
    $newChatProfile.set(target);
    requestFreshSession();
    void ensureGatewayProfile(target);
}
export function setShowAllProfiles(value) {
    $showAllProfiles.set(value);
}
export function toggleShowAllProfiles() {
...
function orderedProfileKeys() {
    ...
    const named = sortByProfileOrder(profiles.filter(profile => !profile.is_default), $profileOrder.get()).map(profile => normalizeProfileKey(profile.name));
...
export function switchToDefaultProfile() {
...
export function switchProfileToSlot(slot) {
    const named = sortByProfileOrder($profiles.get().filter(profile => !profile.is_default), $profileOrder.get());
...
export function cycleProfile(direction) {
...
export function touchActiveGatewayBackend() {
