 ...
 */
export const $todosBySession = atom({});
export const todoListActive = (todos) => todos.some(t => t.status === 'pending' || t.status === 'in_progress');
...
export function todosForHydration(todos) {
...
function cancelScheduledClear(sid) {
...
export function setSessionTodos(sid, todos) {
    if (!sid) {
        return;
    }
    ...
    if (!todoListActive(todos)) {
        clearTimers.set(sid, setTimeout(() => {
            clearTimers.delete(sid);
            clearSessionTodos(sid);
        }, FINISHED_LINGER_MS));
    }
}
export function clearSessionTodos(sid) {
    cancelScheduledClear(sid);
...
export function clearActiveSessionTodos(sid) {
