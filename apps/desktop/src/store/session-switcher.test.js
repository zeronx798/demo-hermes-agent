...
import { $switcherIndex, $switcherOpen, $switcherSessions, closeSwitcher, commitOnCtrlUp, onSwitcherTabDown, onSwitcherTabUp, openOrAdvanceSwitcher, slotSessionId, SWITCHER_REVEAL_MS } from './session-switcher';
const session = (id) => ({ id });
const seed = (ids, selected) => {
    $sessions.set(ids.map(session));
    $selectedStoredSessionId.set(selected);
};
const tabTap = (direction = 1) => {
    onSwitcherTabDown();
