...
const SID = 'sess-1';
const running = (id, command = `cmd ${id}`) => ({ command, session_id: id, status: 'running' });
const exited = (id, exit_code = 0, command = `cmd ${id}`) => ({
    command,
    exit_code,
    session_id: id,
    status: 'exited'
...
describe('reconcileBackgroundProcesses', () => {
    ...
    const itemsOf = (sid) => $backgroundStatusBySession.get()[sid] ?? [];
