...
vi.mock('@/lib/storage', () => ({
    ...
    storedBoolean: (key, fallback) => {
        const value = storage.get(key);
        return value === undefined ? fallback : value === 'true';
    },
    storedString: (key) => storage.get(key) ?? null
}));
const notifySpy = vi.fn();
const dismissSpy = vi.fn();
vi.mock('@/store/notifications', () => ({
    notify: (...args) => notifySpy(...args),
    dismissNotification: (...args) => dismissSpy(...args)
}));
const checkHermesUpdateSpy = vi.fn();
const updateHermesSpy = vi.fn();
const getActionStatusSpy = vi.fn();
vi.mock('@/hermes', () => ({
    checkHermesUpdate: (...args) => checkHermesUpdateSpy(...args),
    updateHermes: (...args) => updateHermesSpy(...args),
    getActionStatus: (...args) => getActionStatusSpy(...args)
}));
const { maybeNotifyUpdateAvailable, checkBackendUpdates, $backendUpdateStatus, applyBackendUpdate, $backendUpdateApply, reportBackendContract, applyUpdates, $updateApply, $updateOverlayOpen, resetUpdateApplyState, startUpdatePoller, stopUpdatePoller, $updateStatus } = await import('./updates');
...
const lastToast = () => notifySpy.mock.calls.at(-1)?.[0];
...
describe('checkBackendUpdates', () => {
    ...
    const setRemote = (on) => setConnection({
