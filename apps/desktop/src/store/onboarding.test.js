...
import { $desktopOnboarding, refreshOnboarding, requestDesktopOnboarding, saveOnboardingLocalEndpoint, submitOnboardingCode } from './onboarding';
...
function runtimeMismatchGateway() {
    return async (method) => {
        if (method === 'setup.status') {
            return { provider_configured: true };
        }
        if (method === 'setup.runtime_check') {
            return { error: 'Selected runtime is not available.', ok: false };
        }
        throw new Error(`unexpected gateway method: ${method}`);
    };
}
function onboardingContext(requestGateway) {
    return { requestGateway };
}
function fallbackTimeoutGateway() {
    return async (method) => {
...
describe('refreshOnboarding', () => {
    ...
    it('deduplicates concurrent provider refresh calls', async () => {
        ...
        const providersPromise = new Promise(resolve => {
...
describe('OAuth onboarding', () => {
    ...
    it('clears stale readiness errors after OAuth succeeds and model confirmation is shown', async () => {
        ...
        const calls = [];
        installApiMock(async ({ body, path }) => {
        ...
        $desktopOnboarding.set(baseState({
            ...
            reason: 'No access token found for Nous Portal login. setup.status reports configured credentials, but runtime resolution still failed.',
            requested: true
        }));
...
describe('saveOnboardingLocalEndpoint', () => {
    ...
    it('auto-discovers the model and persists provider=custom + base_url, then finishes', async () => {
        const calls = [];
        const api = vi.fn(async ({ body, path }) => {
            calls.push({ body, path });
            if (path === '/api/providers/validate') {
    ...
    it('reports the runtime reason when resolution still fails after saving', async () => {
        ...
        const failingGateway = async (method) => {
            if (method === 'reload.env') {
                return {};
            }
            if (method === 'setup.status') {
            ...
            if (method === 'setup.runtime_check') {
                return { ok: false, error: 'No provider can serve the selected model.' };
            }
            throw new Error(`unexpected gateway method: ${method}`);
        };
        const result = await saveOnboardingLocalEndpoint('http://127.0.0.1:8000/v1', '', {
