...
import { $activeSessionId, $attentionSessionIds, $connection, $currentCwd, $workingSessionIds, applyConfiguredDefaultProjectDir, getRecentlySettledSessionIds, mergeSessionPage, sessionPinId, setCurrentCwd, setSessionAttention, setSessionWorking, workspaceCwdForNewSession } from './session';
const session = (over) => ({
    archived: false,
    cwd: null,
    ended_at: null,
...
describe('mergeSessionPage', () => {
    ...
    it('evicts an old compression tip when the incoming page has the new tip from the same lineage', () => {
        ...
        const previous = [session({ id: 'tip-4', _lineage_root_id: 'root' }), session({ id: 'other' })];
        const incoming = [session({ id: 'tip-5', _lineage_root_id: 'root' })];
    ...
    it('preserves an unrelated pinned session even when lineage dedup is active', () => {
        ...
        const incoming = [session({ id: 'a-new', _lineage_root_id: 'lineage-a' })];
        const merged = mergeSessionPage(previous, incoming, ['b']);
...
describe('workspaceCwdForNewSession', () => {
    ...
    it('keeps remote workspace memory separate from local and other remotes', () => {
        ...
        $currentCwd.set('/live/session/path');
        $connection.set({ baseUrl: 'http://backend-a', mode: 'remote' });
        expect(workspaceCwdForNewSession()).toBe('');
        setCurrentCwd('/backend/project-a');
        expect(workspaceCwdForNewSession()).toBe('/backend/project-a');
        $connection.set({ baseUrl: 'http://backend-b', mode: 'remote' });
        expect(workspaceCwdForNewSession()).toBe('');
