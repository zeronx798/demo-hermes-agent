 ...
 */
export function cleanPetName(prompt) {
    const words = prompt
...
export const $petGenStatus = atom('idle');
export const $petGenStage = atom(null);
...
export const $petGenAvailable = atom(null);
...
export function setPetGenProvider(name) {
...
export function markRemixConfirmed() {
...
export async function checkPetGenAvailable(request) {
    try {
...
export function openPetGenerate() {
...
export const $petGenRefImage = atom(null);
 ...
 */
export function cleanupPetGenOnClose(request) {
...
function notifyPetGenDone(title, message, kind) {
...
export async function generateDrafts(request, options) {
    ...
    const off = $gateway.get()?.on('pet.generate.progress', event => {
    ...
    try {
        const result = await request('pet.generate', {
            prompt,
            style: options.style ?? 'auto',
            count: options.count ?? 4,
            ...(referenceImage ? { referenceImage } : {}),
            ...($petGenProvider.get() ? { provider: $petGenProvider.get() } : {})
        }, GENERATE_TIMEOUT_MS, controller.signal);
 ...
 */
export async function hatchSelected(request, options) {
    const token = $petGenToken.get();
    ...
    try {
        const result = await request('pet.hatch', {
            token,
            cancelToken,
            index,
            name,
            ...
            style: options.style ?? 'auto',
            ...($petGenProvider.get() ? { provider: $petGenProvider.get() } : {})
        }, HATCH_TIMEOUT_MS, controller.signal);
 ...
 */
export async function adoptHatched(request, name) {
    const preview = $petGenPreview.get();
    if (!preview?.slug) {
        return { ok: false };
    }
 ...
 */
export async function discardHatched(request) {
    const preview = $petGenPreview.get();
    if (preview?.slug) {
        await request('pet.remove', { slug: preview.slug }).catch(() => { });
    }
