...
import { dispatchNativeNotification, NATIVE_NOTIFICATION_KINDS, respondToApprovalAction, sendTestNativeNotification, setNativeNotifyEnabled, setNativeNotifyKind } from './native-notifications';
import { $approvalRequest, setApprovalRequest } from './prompts';
import { $activeSessionId, setActiveSessionId } from './session';
const desktopWindow = window;
const initialHermesDesktop = desktopWindow.hermesDesktop;
const notify = vi.fn().mockResolvedValue(true);
function setWindowState({ focused = true, hidden = false }) {
...
beforeEach(() => {
    notify.mockClear();
    desktopWindow.hermesDesktop = { notify };
...
describe('dispatchNativeNotification preferences', () => {
    ...
    it('forwards kind and sessionId to the bridge', () => {
        ...
        expect(notify).toHaveBeenCalledWith(expect.objectContaining({ body: 'hi', kind: 'turnError', sessionId: 'abc', title: 'boom' }));
    });
});
