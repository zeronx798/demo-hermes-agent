...
const petRpc = (request, method, params = {}) => request(method, { ...params, profile: petProfile() });
/** A JSON-RPC "method not found" — the backend predates the pet RPCs. */
function isMissingMethod(error) {
    const message = error instanceof Error ? error.message : String(error);
    return /method not found|-32601|unknown method|no such method/i.test(message);
}
export const $petGallery = atom(null);
export const $petGalleryStatus = atom('idle');
export const $petGalleryError = atom(null);
 ...
 */
export function resetPetGallery() {
    galleryLoad = null;
    thumbCache.clear();
    $petGallery.set(null);
...
export function loadPetThumb(request, slug, url) {
 ...
 */
export function loadPetGallery(request, options = {}) {
...
async function syncInfo(request) {
    try {
 ...
 */
export async function applyAdoptedPet(request, slug, displayName) {
 ...
 */
export function rankedGalleryPets(gallery, query = '') {
    if (!gallery) {
        return [];
    }
...
async function mutate(busyKey, fallback, request, run) {
    $petBusy.set(busyKey);
    $petGalleryError.set(null);
    try {
        await run();
...
export function adoptPet(request, slug, fallback) {
 ...
 */
export function setPetEnabled(request, on, copy) {
...
export const clampPetScale = (n) => Math.max(PET_SCALE_MIN, Math.min(PET_SCALE_MAX, n));
 ...
 */
export function nextScaleFromWheel(current, deltaY) {
 ...
 */
export function setPetScale(request, scale) {
...
export async function exportPet(request, slug, fallback) {
    $petBusy.set(slug);
    $petGalleryError.set(null);
    try {
 ...
 */
export function renamePet(request, slug, name, fallback) {
...
export function removePet(request, slug, fallback) {
    return mutate(slug, fallback, request, async () => {
