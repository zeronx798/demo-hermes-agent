...
import { $activeProjectId, $projectScope, $projectsRpcAvailable, $worktreeRefreshToken, ALL_PROJECTS, createProject, enterProject, exitProjectScope, openProjectCreate, pickProjectFolder, refreshProjects, refreshWorktrees } from './projects';
vi.mock('@/i18n', () => ({
...
describe('createProject', () => {
    ...
    it('creates the project and flips into the grouped view so a blank slate shows it', async () => {
        ...
        activeGateway.mockReturnValue({ connectionState: 'open', request });
    ...
    it('marks the backend stale and surfaces a friendly error when projects.create is missing', async () => {
        ...
        await expect(createProject({ folders: ['/srv/demo'], name: 'Demo' })).rejects.toThrow('sidebar.projects.staleBackend');
...
describe('projects RPC capability', () => {
    ...
    it('blocks opening the create dialog once the backend is known stale', () => {
        ...
        openProjectCreate();
        expect(notify).toHaveBeenCalledWith(expect.objectContaining({ kind: 'warning', message: 'sidebar.projects.staleBackend' }));
    });
});
