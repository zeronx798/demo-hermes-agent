...
import { $petActivity, $petInfo, $petUnread, clearPetUnread } from '@/store/pet';
 ...
 */
export function overlayWindowSize(frameW, frameH, scale) {
    return {
 ...
 */
export function popOutPet(petRect) {
 ...
 */
export function restorePetOverlay() {
 ...
 */
export function initPetOverlayBridge() {
    const api = window.hermesDesktop?.petOverlay;
    if (!api || controlUnsub) {
        return () => { };
    }
