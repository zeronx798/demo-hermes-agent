...
export const $repoStatus = atom(null);
...
export const $repoWorktrees = atom([]);
...
export const $repoChangeByPath = computed([$repoStatus, $currentCwd], (status, cwd) => {
    ...
    for (const file of status.files) {
        const kind = file.conflicted ? 'conflicted' : file.untracked ? 'added' : 'modified';
        map.set(`${root}/${file.path}`, kind);
    }
    return map;
});
async function loadWorktrees(target) {
...
let repoStatusRefreshTimer = null;
const normalizeCwd = (cwd) => cwd?.trim() || null;
/**
 ...
 */
export async function refreshRepoStatus(cwd) {
...
function scheduleRepoStatusRefresh(cwd) {
