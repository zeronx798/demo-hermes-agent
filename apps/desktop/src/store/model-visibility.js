...
export const modelVisibilityKey = (provider, model) => `${provider}::${model}`;
...
export const emptyProviderSentinelKey = (provider) => modelVisibilityKey(provider, EMPTY_PROVIDER_SENTINEL);
/** Check whether a stored key is a provider-hidden sentinel. */
export const isProviderSentinel = (key) => key.endsWith('::');
...
export function collapseModelFamilies(models) {
...
export const $visibleModels = atom(loadVisible());
export const $modelVisibilityOpen = atom(false);
export function setVisibleModels(keys) {
    $visibleModels.set(new Set(keys));
    persistString(STORAGE_KEY, JSON.stringify([...keys]));
}
export function setModelVisibilityOpen(open) {
...
export function defaultVisibleKeys(providers) {
    const keys = new Set();
...
function expandProviderDefaults(provider, target) {
...
export function resolveVisibleKeys(stored, providers) {
    if (!stored) {
...
export function effectiveVisibleKeys(stored, providers) {
...
export function toggleModelVisibility(stored, providers, providerSlug, model) {
