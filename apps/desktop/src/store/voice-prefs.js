...
export const $autoSpeakReplies = atom(false);
/** Seed the atom from a loaded config payload (mount / refresh). */
export function applyAutoSpeakFromConfig(config) {
 ...
 */
export async function setAutoSpeakReplies(enabled) {
    ...
    try {
        ...
        const voice = record.voice && typeof record.voice === 'object' ? record.voice : {};
