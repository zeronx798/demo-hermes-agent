...
import { getCuratorStatus, getMcpCatalog, getMemoryStatus, getSkillHubSources, getToolsetModels, installSkillFromHub, resetMemory, runDebugShare, searchSkillsHub, selectToolsetModel, setCuratorPaused, setMcpServerEnabled, testMcpServer } from './hermes';
describe('Hermes REST parity helpers (hub / mcp / maintenance)', () => {
    ...
    it('encodes hub search params', async () => {
        ...
        expect(api).toHaveBeenCalledWith(expect.objectContaining({ path: '/api/skills/hub/search?q=gif+search&source=official&limit=5' }));
    });
    ...
    it('reads memory status and resets a specific target', async () => {
        ...
        expect(api).toHaveBeenNthCalledWith(2, expect.objectContaining({ path: '/api/memory/reset', method: 'POST', body: { target: 'user' } }));
    });
    it('manages the curator', async () => {
        ...
        expect(api).toHaveBeenNthCalledWith(2, expect.objectContaining({ path: '/api/curator/paused', method: 'PUT', body: { paused: true } }));
    });
    it('runs debug share synchronously with an upload-tolerant timeout', async () => {
        await runDebugShare();
        expect(api).toHaveBeenCalledWith(expect.objectContaining({ path: '/api/ops/debug-share', method: 'POST', timeoutMs: 120_000 }));
    });
    it('reads a backend model catalog scoped to a provider row', async () => {
        await getToolsetModels('image_gen', 'FAL.ai');
        expect(api).toHaveBeenCalledWith(expect.objectContaining({ path: '/api/tools/toolsets/image_gen/models?provider=FAL.ai' }));
    });
