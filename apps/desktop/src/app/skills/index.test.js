...
vi.mock('@/hermes', async (importOriginal) => ({
    ...
    toggleSkill: (name, enabled) => toggleSkill(name, enabled),
    toggleToolset: (name, enabled) => toggleToolset(name, enabled),
    getToolsetConfig: (name) => getToolsetConfig(name),
    selectToolsetProvider: (toolset, provider) => selectToolsetProvider(toolset, provider),
    getUsageAnalytics: (days) => getUsageAnalytics(days)
}));
...
function renderSkills() {
    return import('./index').then(({ SkillsView }) => render(_jsx(QueryClientProvider, { client: queryClient, children: _jsx(MemoryRouter, { initialEntries: ['/skills?tab=toolsets'], children: _jsx(SkillsView, {}) }) })));
}
beforeEach(() => {
