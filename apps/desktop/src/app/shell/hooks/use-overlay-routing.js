...
import { AGENTS_ROUTE, appViewForPath, COMMAND_CENTER_ROUTE, isOverlayView, NEW_CHAT_ROUTE, STARMAP_ROUTE } from '@/app/routes';
const SECTIONS = ['sessions', 'system', 'usage'];
export function useOverlayRouting() {
    ...
    const commandCenterInitialSection = useMemo(() => SECTIONS.find(value => value === new URLSearchParams(location.search).get('section')), [location.search]);
    const openCommandCenterSection = useCallback((section) => navigate(`${COMMAND_CENTER_ROUTE}?section=${section}`), [navigate]);
    const closeOverlayToPreviousRoute = useCallback(() => navigate(returnPathRef.current || NEW_CHAT_ROUTE, { replace: true }), [navigate]);
