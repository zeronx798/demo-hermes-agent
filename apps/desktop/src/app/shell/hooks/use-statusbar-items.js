...
export function useStatusbarItems({ agentsOpen, chatOpen, commandCenterOpen, extraLeftItems, extraRightItems, gatewayState, inferenceStatus, openAgents, openCommandCenterSection, freshDraftReady, requestGateway, statusSnapshot, toggleCommandCenter }) {
    ...
    const toggleYolo = useCallback(async (modifiers) => {
    ...
    const gatewayMenuContent = useMemo(() => (close) => (_jsx(GatewayMenuPanel, { gatewayState: gatewayState, inferenceStatus: inferenceStatus, onClose: close, onOpenSystem: () => openCommandCenterSection('system'), statusSnapshot: statusSnapshot })), [gatewayState, inferenceStatus, openCommandCenterSection, statusSnapshot]);
    ...
    const clientVersionItem = useMemo(() => {
        ...
        return {
            ...
            icon: applying ? _jsx(Loader2, { className: "size-3 animate-spin" }) : _jsx(Hash, { className: "size-3" }),
            id: 'version-client',
            label,
    ...
    const backendVersionItem = useMemo(() => {
        ...
        const behindHint = !applying && behind > 0 ? ` (+${behind})` : !applying && updateAvailable ? ` (${copy.update})` : '';
        const label = applying
    ...
    const coreLeftStatusbarItems = useMemo(() => [
        ...
        },
        {
            ...
            icon: gatewayRestarting ? (_jsx(GlyphSpinner, { ariaLabel: copy.gatewayRestarting, className: "size-3" })) : inferenceReady ? (_jsx(Activity, { className: "size-3" })) : (_jsx(AlertCircle, { className: "size-3" })),
            id: 'gateway-health',
            label: copy.gateway,
            menuClassName: 'w-72',
        ...
        {
            className: cn(agentsOpen && 'bg-accent/55 text-foreground', subagentsFailed > 0 && 'text-destructive hover:text-destructive'),
            detail: subagentsRunning > 0
                ? copy.subagents(subagentsRunning)
                : subagentsFailed > 0
                    ? copy.failed(subagentsFailed)
                    : undefined,
            icon: subagentsFailed > 0 ? (_jsx(AlertCircle, { className: "size-3" })) : subagentsRunning > 0 ? (_jsx(Loader2, { className: "size-3 animate-spin" })) : (_jsx(Codicon, { name: "hubot", size: "0.75rem" })),
            id: 'agents',
            label: copy.agents,
            onSelect: openAgents,
    ...
    const coreRightStatusbarItems = useMemo(() => [
        ...
        },
        {
            ...
            icon: yoloActive ? (_jsx(ZapFilled, { className: "size-3.5 shrink-0" })) : (_jsx(Zap, { className: "size-3.5 shrink-0 opacity-70" })),
            id: 'yolo',
