...
import { $previewTarget, clearSessionPreviewRegistry, registerSessionPreview } from '@/store/preview';
...
let handleEvent = () => undefined;
function PreviewRoutingHarness({ onEvent }) {
...
describe('usePreviewRouting', () => {
    beforeEach(() => {
        ...
        Object.defineProperty(window, 'hermesDesktop', {
            configurable: true,
            value: {
                normalizePreviewTarget: vi.fn(async (target) => previewTarget(target))
            }
        });
    });
    afterEach(() => {
    ...
    it('opens the active session preview from the registry', async () => {
        ...
        render(_jsx(PreviewRoutingHarness, { onEvent: handler => {
                handleEvent = handler;
            } }));
        await waitFor(() => {
