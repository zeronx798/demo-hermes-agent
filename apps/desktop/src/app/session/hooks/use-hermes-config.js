...
import { $currentCwd, setAvailablePersonalities, setCurrentCwd, setCurrentFastMode, setCurrentPersonality, setCurrentReasoningEffort, setCurrentServiceTier, setIntroPersonality } from '@/store/session';
import { applyAutoSpeakFromConfig } from '@/store/voice-prefs';
const DEFAULT_VOICE_SECONDS = 120;
const FAST_TIERS = new Set(['fast', 'priority', 'on']);
function recordingLimit(value) {
...
function normalizeConfigEffort(value) {
    if (value === false) {
        return 'none';
    }
...
export function useHermesConfig({ activeSessionIdRef, refreshProjectBranch }) {
    ...
    const refreshHermesConfig = useCallback(async () => {
        try {
            ...
            const personality = normalizePersonalityValue(typeof config.display?.personality === 'string' ? config.display.personality : '');
