...
vi.mock('@/hermes', () => ({
    ...
    setGlobalModel: (...args) => setGlobalModel(...args)
}));
vi.mock('@/i18n', () => ({
    useI18n: () => ({
        t: {
...
function Harness({ activeSessionId, onReady, requestGateway }) {
...
describe('useModelControls', () => {
    ...
    it('routes active-session picker changes through config.set with an explicit provider', async () => {
        const requestGateway = vi.fn(async () => ({ key: 'model', value: 'claude-sonnet-4.6' }));
        let controls;
        render(_jsx(Harness, { activeSessionId: "session-1", onReady: value => (controls = value), requestGateway: requestGateway }));
        await expect(controls.selectModel({
    ...
    it('stores a no-session pick as UI state with no gateway or global write', async () => {
        ...
        let controls;
        render(_jsx(Harness, { activeSessionId: null, onReady: value => (controls = value), requestGateway: requestGateway }));
        await expect(controls.selectModel({
            model: 'claude-sonnet-4.6',
            provider: 'anthropic'
        })).resolves.toBe(true);
