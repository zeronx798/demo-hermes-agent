...
function RouteResumeHarness({ resumeFailedSessionId = null, resumeExhaustedSessionId = null, ...props }) {
...
describe('useRouteResume', () => {
    ...
    it('does not re-resume the old session during a /:sid -> /new transition', () => {
        ...
        const { rerender } = render(_jsx(RouteResumeHarness, { activeSessionId: "runtime-1", activeSessionIdRef: activeSessionIdRef, creatingSessionRef: creatingSessionRef, currentView: "chat", freshDraftReady: false, gatewayState: "open", locationPathname: "/session-1", resumeSession: resumeSession, routedSessionId: "session-1", runtimeIdByStoredSessionIdRef: runtimeIdByStoredSessionIdRef, selectedStoredSessionId: "session-1", selectedStoredSessionIdRef: selectedStoredSessionIdRef, startFreshSessionDraft: startFreshSessionDraft }));
        expect(resumeSession).not.toHaveBeenCalled();
        // Simulate startFreshSessionDraft state updates landing before route update.
        activeSessionIdRef.current = null;
        selectedStoredSessionIdRef.current = null;
        rerender(_jsx(RouteResumeHarness, { activeSessionId: null, activeSessionIdRef: activeSessionIdRef, creatingSessionRef: creatingSessionRef, currentView: "chat", freshDraftReady: true, gatewayState: "open", locationPathname: "/session-1", resumeSession: resumeSession, routedSessionId: "session-1", runtimeIdByStoredSessionIdRef: runtimeIdByStoredSessionIdRef, selectedStoredSessionId: null, selectedStoredSessionIdRef: selectedStoredSessionIdRef, startFreshSessionDraft: startFreshSessionDraft }));
        expect(resumeSession).not.toHaveBeenCalled();
    });
    ...
    it('resumes when pathname changes to a routed session', () => {
        ...
        const selectedStoredSessionIdRef = { current: null };
        const { rerender } = render(_jsx(RouteResumeHarness, { activeSessionId: null, activeSessionIdRef: activeSessionIdRef, creatingSessionRef: creatingSessionRef, currentView: "chat", freshDraftReady: true, gatewayState: "open", locationPathname: "/", resumeSession: resumeSession, routedSessionId: null, runtimeIdByStoredSessionIdRef: runtimeIdByStoredSessionIdRef, selectedStoredSessionId: null, selectedStoredSessionIdRef: selectedStoredSessionIdRef, startFreshSessionDraft: startFreshSessionDraft }));
        expect(resumeSession).not.toHaveBeenCalled();
        rerender(_jsx(RouteResumeHarness, { activeSessionId: null, activeSessionIdRef: activeSessionIdRef, creatingSessionRef: creatingSessionRef, currentView: "chat", freshDraftReady: true, gatewayState: "open", locationPathname: "/session-2", resumeSession: resumeSession, routedSessionId: "session-2", runtimeIdByStoredSessionIdRef: runtimeIdByStoredSessionIdRef, selectedStoredSessionId: null, selectedStoredSessionIdRef: selectedStoredSessionIdRef, startFreshSessionDraft: startFreshSessionDraft }));
        expect(resumeSession).toHaveBeenCalledTimes(1);
...
describe('useRouteResume bounded auto-retry after a failed resume', () => {
    ...
    function strandedProps(resumeSession) {
        return {
    ...
    it('retries the resume on backoff when the routed session is flagged as failed', () => {
        vi.useFakeTimers();
        const resumeSession = vi.fn(async () => undefined);
        render(_jsx(RouteResumeHarness, { ...strandedProps(resumeSession), resumeFailedSessionId: "session-1" }));
    ...
    it('does NOT retry a failed session that is not the routed one', () => {
        ...
        render(_jsx(RouteResumeHarness, { ...strandedProps(resumeSession), resumeFailedSessionId: "other-session" }));
    ...
    it('skips the scheduled retry if the session already recovered when the timer fires', () => {
        ...
        render(_jsx(RouteResumeHarness, { ...props, resumeFailedSessionId: "session-1" }));
        resumeSession.mockClear(); // drop the mount resume
    ...
    it('stops retrying after MAX_RESUME_RETRIES consecutive failures', () => {
        ...
        const { rerender } = render(_jsx(RouteResumeHarness, { ...props, resumeFailedSessionId: "session-1" }));
        ...
        for (let i = 0; i < 8; i += 1) {
            ...
            rerender(_jsx(RouteResumeHarness, { ...props, resumeFailedSessionId: null })); // cleared at entry
            rerender(_jsx(RouteResumeHarness, { ...props, resumeFailedSessionId: "session-1" })); // re-armed on failure
        }
    ...
    it('does not arm the exhausted latch while retries remain', () => {
        ...
        for (let i = 0; i < 2; i += 1) {
            vi.advanceTimersByTime(8_000);
            rerender(_jsx(RouteResumeHarness, { ...props, resumeFailedSessionId: null }));
            rerender(_jsx(RouteResumeHarness, { ...props, resumeFailedSessionId: "session-1" }));
        }
    ...
    it('clears a stale exhausted latch when the route moves off the stranded session', () => {
        ...
        render(_jsx(RouteResumeHarness, { ...props, activeSessionId: "runtime-2", activeSessionIdRef: { current: 'runtime-2' }, locationPathname: "/session-2", resumeFailedSessionId: null, routedSessionId: "session-2", selectedStoredSessionId: "session-2", selectedStoredSessionIdRef: { current: 'session-2' } }));
        expect($resumeExhaustedSessionId.get()).toBeNull();
    });
    it('resets the retry counter for a fresh backoff cycle when the exhausted latch clears (manual retry, same session)', () => {
        ...
        resumeSession.mockClear();
        rerender(_jsx(RouteResumeHarness, { ...props, resumeExhaustedSessionId: "session-1", resumeFailedSessionId: "session-1" }));
        rerender(_jsx(RouteResumeHarness, { ...props, resumeExhaustedSessionId: null, resumeFailedSessionId: null }));
        rerender(_jsx(RouteResumeHarness, { ...props, resumeExhaustedSessionId: null, resumeFailedSessionId: "session-1" }));
    ...
    it('does not burn retry attempts on unrelated re-renders during the backoff window', () => {
        ...
        const { rerender } = render(_jsx(RouteResumeHarness, { ...props, resumeFailedSessionId: "session-1", resumeSession: vi.fn(async () => undefined) }));
        for (let j = 0; j < 8; j += 1) {
            rerender(_jsx(RouteResumeHarness, { ...props, resumeFailedSessionId: "session-1", resumeSession: vi.fn(async () => undefined) }));
        }
        expect($resumeExhaustedSessionId.get()).toBeNull();
    });
});
