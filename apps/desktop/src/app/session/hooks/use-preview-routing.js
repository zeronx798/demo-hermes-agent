...
import { $previewTarget, $sessionPreviewRegistry, beginPreviewServerRestart, completePreviewServerRestart, getSessionPreviewRecord, progressPreviewServerRestart, requestPreviewReload, setPreviewTarget } from '@/store/preview';
import { $currentCwd } from '@/store/session';
function asRecord(payload) {
    return payload && typeof payload === 'object' ? payload : {};
}
function activePreviewSessionId(activeSessionIdRef, routedSessionId, selectedStoredSessionId) {
    return selectedStoredSessionId || routedSessionId || activeSessionIdRef.current || '';
}
export function usePreviewRouting({ activeSessionIdRef, baseHandleGatewayEvent, currentCwd, currentView, requestGateway, routedSessionId, selectedStoredSessionId }) {
    ...
    const restartPreviewServer = useCallback(async (url, context) => {
    ...
    const handleDesktopGatewayEvent = useCallback(event => {
