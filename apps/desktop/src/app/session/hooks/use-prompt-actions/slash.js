...
import { desktopSlashUnavailableMessage, isDesktopSlashCommand, resolveDesktopCommand } from '@/lib/desktop-slash-commands';
...
import { $connection, $sessions, $yoloActive, setModelPickerOpen, setSessionPickerOpen, setSessions, setYoloActive } from '@/store/session';
import { isSessionIdCandidate, renderCommandsCatalog, slashStatusText } from './utils';
/** The /slash command dispatcher, extracted from usePromptActions. */
export function useSlashCommand(deps) {
    const { activeSessionIdRef, appendSessionTextMessage, branchCurrentSession, busyRef, copy, createBackendSessionForSend, handleSkinCommand, handoffSession, openMemoryGraph, refreshSessions, requestGateway, resumeStoredSession, startFreshSessionDraft, submitPromptText } = deps;
    return useCallback(async (rawCommand, options) => {
        const ensureSessionId = async (sessionHint) => sessionHint || activeSessionIdRef.current || (await createBackendSessionForSend());
        ...
        const withSlashOutput = async (ctx) => {
            const sessionId = await ensureSessionId(ctx.sessionHint);
            if (!sessionId) {
                notify({ kind: 'error', title: copy.sessionUnavailable, message: copy.createSessionFailed });
                return null;
            }
            const render = (text) => appendSessionTextMessage(sessionId, 'system', ctx.recordInput ? slashStatusText(ctx.command, text) : text);
        ...
        async function runExec(ctx) {
            ...
            const handleDispatch = async (dispatch) => {
                ...
                if (!message) {
                    renderSlashOutput(`/${name}: ${dispatch.type === 'skill' ? 'skill payload missing message' : 'empty message'}`);
                    return;
                }
            ...
            try {
                const dispatch = parseCommandDispatch(await requestGateway('command.dispatch', { session_id: sessionId, name, arg }));
                if (!dispatch) {
        ...
        const actionHandlers = {
            ...
            browser: async (ctx) => {
                ...
                if ($connection.get()?.mode === 'remote') {
                    renderSlashOutput('/browser manages a Chromium-family browser on the gateway host — only available when connected to a local gateway.');
                    return;
                }
                ...
                if (!['connect', 'disconnect', 'status'].includes(cmdAction)) {
                    renderSlashOutput('usage: /browser [connect|disconnect|status] [url] · persistent: set browser.cdp_url in config.yaml');
                    return;
                }
                ...
                try {
                    ...
                    if (cmdAction === 'status') {
                        renderSlashOutput(result?.connected
        ...
        const openPicker = async (pickerId, ctx) => {
            if (pickerId === 'model') {
                if (!ctx.arg.trim()) {
            ...
            const match = sessions.find(session => session.id === query) ||
        ...
        async function runSlash(commandText, sessionHint, recordInput = true) {
            ...
            const ctx = { arg, command, name, recordInput, sessionHint };
