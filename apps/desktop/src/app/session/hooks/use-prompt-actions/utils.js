...
export function isSessionIdCandidate(value) {
...
export async function withSessionBusyRetry(call) {
...
export const _submitInFlight = new Set();
export function base64FromDataUrl(dataUrl) {
    const comma = dataUrl.indexOf(',');
    return comma >= 0 ? dataUrl.slice(comma + 1) : '';
}
export function imageFilenameFromPath(filePath) {
...
export async function readImageForRemoteAttach(filePath) {
...
export async function readFileDataUrlForAttach(filePath) {
...
// through unchanged.
export function friendlyRemoteAttachError(err, label) {
...
export function renderCommandsCatalog(catalog, copy) {
...
export function slashStatusText(command, output) {
...
export function visibleUserOrdinal(messages, end) {
    return messages.slice(0, end).filter(m => m.role === 'user' && !m.hidden).length;
}
export function visibleUserIndexAtOrdinal(messages, targetOrdinal) {
    let ordinal = 0;
