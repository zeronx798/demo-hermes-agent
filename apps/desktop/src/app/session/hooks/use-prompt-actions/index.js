...
import { appendText, blobToDataUrl, delay, friendlyRemoteAttachError, inlineErrorMessage, isSessionBusyError, isSessionNotFoundError, readFileDataUrlForAttach, readImageForRemoteAttach, visibleUserIndexAtOrdinal, visibleUserOrdinal, withSessionBusyRetry } from './utils';
/**
...
export function usePromptActions({ activeSessionId, activeSessionIdRef, busyRef, branchCurrentSession, createBackendSessionForSend, handleSkinCommand, openMemoryGraph, refreshSessions, requestGateway, resumeStoredSession, selectedStoredSessionIdRef, startFreshSessionDraft, sttEnabled, updateSessionState }) {
    const { t } = useI18n();
    ...
    const cancelRun = useCallback(async () => {
        ...
        const finalizeMessages = (messages, streamId) => messages
    ...
    const reloadFromMessage = useCallback(async (parentId) => {
        ...
        updateSessionState(activeSessionId, state => {
            const nextUserIndex = state.messages.findIndex((message, index) => index > absoluteUserIndex && message.role === 'user');
    ...
    const submitRewindPrompt = useCallback(async (sessionId, text, truncateOrdinal, interruptFirst) => {
        ...
        const submit = () => requestGateway('prompt.submit', {
            session_id: sessionId,
            text,
    ...
    const restoreToMessage = useCallback(async (messageId, target) => {
        ...
        const truncateBeforeUserOrdinal = target?.userOrdinal === null || target?.userOrdinal === undefined
    ...
    const editMessage = useCallback(async (edited) => {
        ...
        const editedMessage = { ...source, parts: [textPart(text)] };
        ...
        try {
            await submitRewindPrompt(sessionId, text, isFailedTurn ? undefined : visibleUserOrdinal(messages, sourceIndex), busyRef.current || $busy.get());
        }
        catch (err) {
            let surfaced = err;
