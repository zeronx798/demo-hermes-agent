...
const ringRadius = (i) => RING_CORE + i * RING_BAND;
...
const populatedStarts = (stamps, u) => [...new Set(stamps.map(t => bucketStart(t, u)))].sort((a, b) => a - b);
...
function evenLayout(recById, minTs, maxTs, timed) {
    const rings = Array.from({ length: RING_STEPS + 1 }, (_, i) => ({
        label: timed && minTs !== null && maxTs !== null
...
export function buildSimulation(graph, onTick) {
    ...
    const sim = forceSimulation(nodes)
        ...
        .iterations(2))
        .force('radial', forceRadial(n => n.tr, 0, 0).strength(0.92))
        .on('tick', onTick);
