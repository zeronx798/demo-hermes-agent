...
import { LIT_BAND_ALPHA, NODE_SHAPE, ORB_DARKEN, RING_INNER, RING_PARAMS, TILT, WHITE, WHITEISH_SHEEN } from './constants';
...
export function drawScene(scene) {
    const { adjacency, byId, ctx, dpr, fades, focusId, hoverId, hoverLink, hoverRing, links, memById, nodes, palette, reveal, rings, selectedRing, size, snapMotion = false, vp } = scene;
    // Small epsilon so a node exactly at the playhead counts as revealed.
    const seen = (rec) => rec <= reveal + 1e-3;
...
export function drawScramble({ ctx, dpr, palette, rings, vp }) {
