...
// header here.
export function StarmapView({ onClose }) {
    const { t } = useI18n();
    ...
    return (_jsx(Panel, { closeLabel: t.starmap.close, onClose: onClose, children: error ? (_jsx(PanelEmpty, { description: error, icon: "warning", title: t.starmap.loadFailed })) : !shown && loading ? (_jsx(PageLoader, { "aria-label": t.starmap.loading, className: "min-h-0 flex-1" })) : shown && shown.nodes.length === 0 && !imported ? (_jsx(PanelEmpty, { description: t.starmap.emptyDesc, icon: "lightbulb", title: t.starmap.emptyTitle })) : shown ? (_jsx(StarMap, { graph: shown, imported: imported !== null, onImport: setImported, onResetMap: () => setImported(null) })) : null }));
}
