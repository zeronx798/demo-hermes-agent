import { createLoadout, Dict, idxOf, indexBits, LoadoutError } from '@/lib/loadout';
// ── Star-map share code ───────────────────────────────────────────────────────
//
...
const trim = (s) => (s.length > MAX_LABEL ? s.slice(0, MAX_LABEL) : s);
const KINDS = ['skill', 'memory'];
const STATES = ['active', 'archived', 'disabled', 'draft'];
const MEM_SOURCES = ['none', 'memory', 'profile'];
const CREATED_BY = ['none', 'agent', 'user'];
const REC_BITS = 12; // time position resolution: 1/4096 of the span — sub-pixel here.
const REC_MAX = (1 << REC_BITS) - 1;
const finiteTs = (v) => typeof v === 'number' && Number.isFinite(v) ? Math.max(0, Math.round(v)) : null;
...
function readNode(r, dict, i, minTs, span) {
...
export function encodeShareCode(graph) {
