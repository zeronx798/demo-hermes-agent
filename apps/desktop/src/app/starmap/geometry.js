...
export function recencyInk(rec) {
...
export function fitViewport(w, h, outer = RING_OUTER) {
    if (w <= 0 || h <= 0) {
...
export function radiusForRecency(rec, outer = RING_OUTER) {
...
export const fitScale = (w, h, rings) => fitViewport(w, h, rings.at(-1)?.r ?? RING_OUTER).k;
