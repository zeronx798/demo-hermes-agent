...
vi.mock('@/hermes', () => ({
    getMemoryProviderConfig: (provider) => getMemoryProviderConfig(provider),
    saveMemoryProviderConfig: (provider, values) => saveMemoryProviderConfig(provider, values)
}));
vi.mock('@/store/notifications', () => ({
    notify: vi.fn(),
    notifyError: vi.fn()
}));
function hindsightSchema(overrides = []) {
    const fields = [
        {
            key: 'mode',
            label: 'Mode',
...
async function renderPanel(provider = 'hindsight') {
    ...
    return render(_jsx(ProviderConfigPanel, { provider: provider }));
}
describe('ProviderConfigPanel', () => {
    ...
    it('saves edited values without requiring a secret replacement', async () => {
        ...
        await waitFor(() => expect(saveMemoryProviderConfig).toHaveBeenCalledWith('hindsight', {
            mode: 'cloud',
            api_key: '',
