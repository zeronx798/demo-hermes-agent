...
vi.mock('@/hermes', () => ({
    getToolsetConfig: (name) => getToolsetConfig(name),
    getToolsetModels: (name, provider) => getToolsetModels(name, provider),
    selectToolsetModel: (name, model, provider) => selectToolsetModel(name, model, provider),
    selectToolsetProvider: (name, provider) => selectToolsetProvider(name, provider),
    setEnvVar: (key, value) => setEnvVar(key, value),
    deleteEnvVar: (key) => deleteEnvVar(key),
    revealEnvVar: (key) => revealEnvVar(key),
    runToolsetPostSetup: (name, key) => runToolsetPostSetup(name, key),
    getActionStatus: (name, lines) => getActionStatus(name, lines)
}));
...
describe('ToolsetConfigPanel', () => {
    it('lists providers from the config endpoint', async () => {
        ...
        render(_jsx(ToolsetConfigPanel, { onConfiguredChange: vi.fn(), toolset: "tts" }));
    ...
    it('shows a backend model catalog for image_gen and persists a pick', async () => {
        ...
        const { ToolsetConfigPanel } = await import('./toolset-config-panel');
        render(_jsx(ToolsetConfigPanel, { onConfiguredChange: vi.fn(), toolset: "image_gen" }));
    ...
    it('runs a provider post-setup install hook and tails its log', async () => {
        ...
        const { ToolsetConfigPanel } = await import('./toolset-config-panel');
        render(_jsx(ToolsetConfigPanel, { onConfiguredChange: vi.fn(), toolset: "browser" }));
