...
vi.mock('@/hermes', () => ({
    disconnectOAuthProvider: (providerId) => disconnectOAuthProvider(providerId),
    getEnvVars: () => getEnvVars(),
    listOAuthProviders: () => listOAuthProviders()
}));
vi.mock('@/store/onboarding', () => ({
    $desktopOnboarding: onboarding,
    startManualProviderOAuth: (providerId) => startManualProviderOAuth(providerId)
}));
function provider(id, loggedIn, patch = {}) {
    return {
...
async function renderProvidersSettings() {
    ...
    return render(_jsx(ProvidersSettings, { onClose: vi.fn(), onViewChange: vi.fn(), view: "accounts" }));
}
describe('ProvidersSettings', () => {
    ...
    it('renders a Keys card for a backend-tagged provider with no PROVIDER_GROUPS prefix', async () => {
        ...
        const { ProvidersSettings } = await import('./providers-settings');
        render(_jsx(ProvidersSettings, { onClose: vi.fn(), onViewChange: vi.fn(), view: "keys" }));
