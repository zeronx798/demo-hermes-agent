...
export const KEYS_VIEWS = ['tools', 'settings'];
...
export function KeysSettings({ view }) {
    const { t } = useI18n();
    ...
    if (!vars) {
        return _jsx(LoadingState, { label: t.settings.keys.loading });
    }
    const visible = groups.filter(g => g.category === view);
    return (_jsxs(SettingsContent, { children: [visible.map(group => (_jsx("div", { className: "grid gap-2", children: group.entries.map(([key, info]) => {
                    const label = credentialRowLabel(key, info);
                    return (_jsx(CredentialKeyCard, { expanded: openKey === key, info: info, label: label, onExpand: () => setOpenKey(key), onToggle: () => setOpenKey(prev => (prev === key ? null : key)), placeholder: credentialPlaceholder(key, info, label), rowProps: rowProps, varKey: key }, key));
                }) }, group.category))), visible.length === 0 && (_jsx("div", { className: "rounded-lg border border-dashed border-(--ui-stroke-tertiary) px-4 py-8 text-center text-[length:var(--conversation-caption-font-size)] text-muted-foreground", children: t.settings.keys.empty }))] }));
}
