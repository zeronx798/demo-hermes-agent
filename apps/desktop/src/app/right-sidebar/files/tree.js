...
import { $repoChangeByPath } from '@/store/coding-status';
...
function withTreeInset(paddingLeft) {
...
export function ProjectTree({ collapseNonce, cwd, data, onActivateFile, onActivateFolder, onLoadChildren, onNodeOpenChange, onPreviewFile, openState }) {
    ...
    const revealNode = useCallback(async (absPath) => {
    ...
    useEffect(() => $revealInTreeRequest.subscribe(path => {
        if (!path) {
            return;
        }
    ...
    const handleRenameShortcut = useCallback((event) => {
    ...
    return (_jsx("div", { className: "min-h-0 flex-1 overflow-hidden", onKeyDownCapture: handleRenameShortcut, ref: containerRef, children: size.height > 0 && size.width > 0 ? (_jsx(Tree, { childrenAccessor: node => (node?.isDirectory ? (node.children ?? []) : null), data: data, disableDrag: true, disableDrop: true, disableEdit: true, dndManager: getFileTreeDndManager(), height: size.height, indent: INDENT, initialOpenState: openState, onActivate: handleActivate, onToggle: handleToggle, openByDefault: false, padding: 0, ref: treeRef, renderRow: ProjectTreeRowContainer, rowHeight: ROW_HEIGHT, width: size.width, children: props => (_jsx(ProjectTreeRow, { ...props, changeKind: props.node.data ? changeByPath.get(props.node.data.id) : undefined, onAttachFile: onActivateFile, onAttachFolder: onActivateFolder, onPreviewFile: onPreviewFile, relativeTo: cwd })) }, `${cwd}:${collapseNonce}`)) : (_jsx(TreeSizingState, {})) }));
}
function TreeSizingState() {
    return _jsx(TreeSkeleton, {});
}
...
function ProjectTreeRowContainer({ attrs, children, innerRef, node }) {
    return (_jsx("div", { ...attrs, onClick: node.handleClick, onFocus: e => e.stopPropagation(), ref: innerRef, style: { ...attrs.style, minWidth: 0, width: '100%' }, children: children }));
}
const CHANGE_TINT = {
    added: 'text-(--ui-green)',
...
function ProjectTreeRow({ changeKind, dragHandle, node, onAttachFile, onAttachFolder, onPreviewFile, relativeTo, style }) {
    ...
    const row = (_jsxs("div", { "aria-expanded": isFolder ? node.isOpen : undefined, "aria-selected": node.isSelected, className: cn('group/row row-hover flex h-full select-none items-center gap-1 border border-transparent px-3 text-xs font-normal leading-(--file-tree-row-height) text-(--ui-text-secondary) hover:text-foreground', node.isSelected && 'bg-(--ui-row-active-background) text-foreground', isPlaceholder && 'pointer-events-none italic text-muted-foreground/70'), draggable: !isPlaceholder && !editing, onClick: event => {
            event.stopPropagation();
        ...
        }, ref: dragHandle, style: {
            ...style,
            paddingLeft: withTreeInset(style.paddingLeft)
        }, title: node.data.id, children: [_jsx("span", { "aria-hidden": true, className: "flex w-3.5 items-center justify-center text-(--ui-text-tertiary)", children: isPlaceholder && !isErrorPlaceholder ? (_jsx(Codicon, { name: "loading", size: "0.75rem", spinning: true })) : isErrorPlaceholder ? (_jsx(Codicon, { name: "warning", size: "0.75rem" })) : isFolder ? (_jsx(Codicon, { name: node.isOpen ? 'folder-opened' : 'folder', size: "0.875rem" })) : (_jsx(Codicon, { name: "file", size: "0.875rem" })) }), editing ? (_jsx(InlineRenameInput, { name: node.data.name, path: node.data.id })) : (_jsx("span", { className: cn('min-w-0 flex-1 truncate', changeKind && CHANGE_TINT[changeKind]), children: node.data.name }))] }));
    if (isPlaceholder) {
        return row;
    }
    return (_jsx(FileEntryContextMenu, { isDirectory: isFolder, name: node.data.name, path: node.data.id, relativeTo: relativeTo, children: row }));
}
