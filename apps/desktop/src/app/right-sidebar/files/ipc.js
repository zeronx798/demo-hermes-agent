...
function relativeTo(root, child) {
    const r = clean(root);
    const c = clean(child);
    if (c === r) {
        return '';
...
function ancestorDirs(root, dir) {
    const r = clean(root);
...
async function gitignoreFor(dir) {
...
function ignoredBy(rules, entry) {
...
async function filterIgnored(entries, rootPath, dirPath) {
    const root = await gitRootFor(rootPath);
    if (!root) {
        return entries;
    }
    const rules = (await Promise.all(ancestorDirs(root, dirPath).map(gitignoreFor))).filter((r) => Boolean(r));
    return rules.length > 0 ? entries.filter(entry => !ignoredBy(rules, entry)) : entries;
}
export async function readProjectDir(dirPath, rootPath = dirPath) {
...
export function clearProjectDirCache(rootPath) {
    if (!rootPath) {
        gitRootCache.clear();
