...
function makeNode(path, name, isDirectory) {
...
function setProjectTree(updater) {
...
async function fallbackRootFor(cwd) {
...
async function loadRoot(cwd, { force = false } = {}) {
    if (!cwd) {
        clearProjectTree();
        return;
    }
...
async function revalidateTree(cwd) {
    ...
    clearProjectDirCache();
    const reconcile = async (dirPath, existing) => {
 ...
 */
export function useProjectTree(cwd) {
    ...
    const setNodeOpen = useCallback((id, open) => {
