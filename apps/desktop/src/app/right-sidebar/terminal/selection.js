...
// crisp against it.
export function terminalTheme(mode, palette) {
...
export function resolveSurfaceColor(fallback) {
...
export function isAddSelectionShortcut(event) {
    const mod = isMacPlatform() ? event.metaKey : event.ctrlKey;
    return mod && !event.shiftKey && event.key.toLowerCase() === 'l';
}
export function terminalSelectionLabel(term, shellName, text) {
...
export function terminalSelectionAnchor(host) {
    const rect = Array.from(host.querySelectorAll('.xterm-selection div'))
