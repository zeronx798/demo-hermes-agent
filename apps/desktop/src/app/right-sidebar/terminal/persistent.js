 ...
 */
const $slot = atom(null);
const SLOT_CLASS = 'relative flex min-h-0 min-w-0 flex-1 flex-col';
export function TerminalSlot({ className = SLOT_CLASS }) {
    ...
    return _jsx("div", { className: className, ref: ref });
}
const sameRect = (a, b) => !!a && a.top === b.top && a.left === b.left && a.width === b.width && a.height === b.height;
export function PersistentTerminal({ onAddSelectionToChat }) {
    ...
    useLayoutEffect(() => {
        ...
        const tick = () => {
            ...
            const next = { top, left, width: Math.ceil(r.right) - left, height: Math.ceil(r.bottom) - top };
    ...
    return (_jsx("div", { "aria-hidden": !visible, style: style, children: mounted && _jsx(TerminalWorkspace, { onAddSelectionToChat: onAddSelectionToChat }) }));
}
