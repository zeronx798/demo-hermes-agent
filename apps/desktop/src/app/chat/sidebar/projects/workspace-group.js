...
export function SidebarWorkspaceGroup({ group, renderRows, onNewSession, onRemove }) {
    const { t } = useI18n();
    const s = t.sidebar;
    ...
    const leadingIcon = group.color ? (_jsx("span", { "aria-hidden": "true", className: "size-2 shrink-0 rounded-full", style: { backgroundColor: group.color } })) : (_jsx(Codicon, { className: "shrink-0 text-(--ui-text-tertiary)", name: group.isKanban ? 'checklist' : group.isHome ? 'home' : 'git-branch', size: "0.75rem" }));
    ...
    return (_jsxs(SidebarRowStack, { children: [_jsx(WorkspaceHeader, { action: (onNewSession || isProfileGroup || onRemove) && (_jsxs("div", { className: "flex items-center", children: [(onNewSession || isProfileGroup) && (_jsx(WorkspaceAddButton, { label: s.newSessionIn(group.label), 
                            ...
                            onClick: () => void handleNewSession() })), onRemove && _jsx(WorkspaceMenu, { onRemove: onRemove, path: group.path })] })), count: isProfileGroup ? countLabel(visibleSessions.length, totalCount) : group.sessions.length, icon: leadingIcon, label: group.label, onToggle: toggleOpen, open: open, title: group.path ?? undefined }), open && (_jsxs(_Fragment, { children: [visibleSessions.length === 0 ? (_jsx("div", { className: "min-h-7 pl-2 text-[0.75rem] leading-7 text-(--ui-text-quaternary)", children: s.noSessions })) : (renderRows(visibleSessions)), hiddenCount > 0 &&
                        (isProfileGroup ? (_jsx(SidebarLoadMoreRow, { loading: Boolean(group.loadingMore), onClick: handleProfileLoadMore, step: nextCount })) : (_jsx(WorkspaceShowMoreButton, { count: nextCount, label: group.label, onClick: () => setVisibleCount(count => count + SIDEBAR_GROUP_PAGE) })))] }))] }));
}
