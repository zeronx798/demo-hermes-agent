...
/** Last path segment. */
export const baseName = (path) => segments(path).pop();
...
export function kanbanWorktreeDir(path) {
...
export const branchLaneId = (repoRoot, branch) => `${repoRoot}::branch::${(branch ?? '').trim()}`;
/** A session's recency stamp (last activity, falling back to creation). */
export const sessionRecency = (session) => session.last_active || session.started_at || 0;
...
const isTrunkLane = (group) => Boolean(group.isMain) && TRUNK_BRANCHES.has(group.label.toLowerCase());
/** A lane's recency = its most-recently-active session (empty lanes sink). */
const laneActivity = (group) => group.sessions.reduce((max, session) => Math.max(max, sessionRecency(session)), 0);
...
const laneRank = (group) => group.isHome ? 0 : isTrunkLane(group) ? 1 : group.isKanban ? 3 : 2;
/**
 ...
 */
function compareWorktreeGroups(a, b) {
...
export function sortWorktreeGroups(groups) {
 ...
 */
export function mergeRepoWorktreeGroups(repo, discoveredWorktrees) {
    ...
    const liveBranchByPath = new Map();
    ...
    const livePathByBranch = new Map();
    ...
    const seenPaths = new Set(merged.map(group => group.path).filter((path) => Boolean(path)));
    ...
    for (const worktree of discoveredWorktrees ?? []) {
        ...
        const label = (worktree.isMain ? worktree.branch?.trim() || DEFAULT_BRANCH_LABEL : worktree.branch?.trim()) ||
            baseName(wtPath) ||
            wtPath;
        const id = worktree.isMain ? branchLaneId(repo.id, label) : wtPath;
        const alreadySeen = seenIds.has(id) || seenLabels.has(label.toLowerCase()) || (!worktree.isMain && seenPaths.has(wtPath));
        if (alreadySeen) {
            continue;
        }
 ...
 */
export function liveSessionProjectId(session, explicitProjects) {
...
const upsertSession = (rows, session) => [session, ...rows.filter(row => row.id !== session.id)].sort((a, b) => b.started_at - a.started_at);
/**
 ...
 */
function liveLaneForRepo(repoRoot, session) {
    const cwd = (session.cwd || '').trim();
 ...
 */
export function overlayRepoLanes(repo, live, removed = NO_REMOVED) {
...
export function overlayLiveLanes(project, live, removed = NO_REMOVED) {
    let changed = false;
...
export function overlayLivePreviews(projects, live, explicitProjects, limit, removed = new Set()) {
