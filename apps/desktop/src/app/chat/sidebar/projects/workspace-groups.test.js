...
import { baseName, kanbanWorktreeDir, liveSessionProjectId, mergeRepoWorktreeGroups, overlayLiveLanes, overlayLivePreviews, sortWorktreeGroups } from './workspace-groups';
...
describe('sortWorktreeGroups', () => {
    it('pins trunk to the top, sinks kanban to the bottom, and orders the rest by recency', () => {
        const at = (t) => [makeSession('/x', { last_active: t })];
        const groups = [
...
describe('liveSessionProjectId', () => {
    ...
    it('places an in-tree worktree session under its repo project (the root is in the path)', () => {
        ...
        expect(liveSessionProjectId(makeSession('/www/app/.worktrees/test1', { git_repo_root: '/www/app' }), [])).toBe('/www/app');
    });
