...
const pathListKey = (paths) => paths
    .map(path => path.trim())
    .filter(Boolean)
    .sort((a, b) => a.localeCompare(b))
    .join('\n');
// Every session in a project, across its repos/worktrees (order-agnostic).
const projectSessions = (project) => project.repos.flatMap(repo => repo.groups.flatMap(group => group.sessions));
export const projectTreeCwd = (project) => project.path || project.repos.find(repo => repo.path)?.path || null;
...
const projectActivityTime = (project) => Math.max(project.lastActive ?? 0, projectSessions(project).reduce((latest, s) => Math.max(latest, sessionRecency(s)), 0));
// The project's most-recent sessions, for the overview preview under each row.
export const latestProjectSessions = (project, limit) => [...projectSessions(project)].sort((a, b) => sessionRecency(b) - sessionRecency(a)).slice(0, limit);
export function sortProjectsForOverview(projects, activeProjectId) {
...
export function useRepoWorktreeMap(repoPaths, enabled) {
    ...
    useEffect(() => {
        ...
        void mapPool(repoPaths, WORKTREE_PROBE_CONCURRENCY, async (repoPath) => {
            try {
                return [repoPath, await git.worktreeList(repoPath)];
            }
            catch {
                return [repoPath, []];
            }
...
export function useWorkspaceNodeOpen(id, defaultOpen = true) {
