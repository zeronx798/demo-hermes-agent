...
export function reconcileFreshFirst(currentIds, orderIds) {
    const current = new Set(currentIds);
    const retained = orderIds.filter(id => current.has(id));
    const retainedSet = new Set(retained);
    return [...currentIds.filter(id => !retainedSet.has(id)), ...retained];
}
export function resolveManualSessionOrderIds(currentIds, orderIds, manual) {
...
export function orderByIds(items, getId, orderIds) {
    if (!orderIds.length) {
        return items;
    }
...
export function reconcileOrderIds(currentIds, orderIds) {
