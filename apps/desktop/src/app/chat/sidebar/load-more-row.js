...
export function SidebarLoadMoreRow({ step, onClick, loading = false }) {
    const { t } = useI18n();
    const label = loading ? t.sidebar.loading : step > 0 ? t.sidebar.loadCount(step) : t.sidebar.loadMore;
    return (_jsx("button", { "aria-label": label, className: "ml-auto grid size-5 place-items-center rounded-sm bg-transparent text-(--ui-text-tertiary) transition-colors hover:bg-(--ui-control-hover-background) hover:text-foreground disabled:cursor-default disabled:opacity-60 disabled:hover:bg-transparent disabled:hover:text-(--ui-text-tertiary)", disabled: loading, onClick: onClick, type: "button", children: loading ? (_jsx(GlyphSpinner, { ariaLabel: label, className: "text-[0.75rem]" })) : (_jsx(Codicon, { name: "ellipsis", size: "0.75rem" })) }));
}
