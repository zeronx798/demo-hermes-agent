...
function Harness({ onPayload }) {
    ...
    return (_jsx("div", { contentEditable: true, "data-testid": "editor", onCompositionEnd: event => {
            ...
            flushEditorToDraft(event.currentTarget);
        }, ref: editorRef, suppressContentEditableWarning: true }));
}
describe('composer IME composition — send button visibility (#39614)', () => {
    it('shows the send button after committing CJK text without a trailing edit', async () => {
        ...
        const { getByTestId } = render(_jsx(Harness, { onPayload: p => (hasPayload = p) }));
