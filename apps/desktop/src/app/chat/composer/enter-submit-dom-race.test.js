...
function Harness({ busy = false, disabled = false, queued = [], onSubmit, onQueue, onCancel, onDrain }) {
    ...
    const attachments = [];
    const composerPlainText = (el) => el.textContent ?? '';
    const setText = (next) => {
    ...
    void draft;
    return (_jsx("div", { contentEditable: true, "data-testid": "editor", onInput: event => setText(composerPlainText(event.currentTarget)), onKeyDown: handleKeyDown, ref: editorRef, suppressContentEditableWarning: true }));
}
describe('composer Enter submit — live DOM vs stale composer state (#39630)', () => {
    it('sends the just-typed text on Enter even when composer state has not synced', async () => {
        ...
        const { getByTestId } = render(_jsx(Harness, { onCancel: vi.fn(), onDrain: vi.fn(), onQueue: vi.fn(), onSubmit: onSubmit }));
    ...
    it('queues a fast-typed message while busy instead of draining the queue or cancelling', async () => {
        ...
        const { getByTestId } = render(_jsx(Harness, { busy: true, onCancel: onCancel, onDrain: onDrain, onQueue: onQueue, onSubmit: vi.fn(), queued: ['queued-1'] }));
        const editor = getByTestId('editor');
        await act(async () => {
    ...
    it('treats an empty Enter while busy as a no-op (never an accidental Stop)', async () => {
        ...
        const onSubmit = vi.fn();
        const onQueue = vi.fn();
        const { getByTestId } = render(_jsx(Harness, { busy: true, onCancel: onCancel, onDrain: vi.fn(), onQueue: onQueue, onSubmit: onSubmit }));
        const editor = getByTestId('editor');
        await act(async () => {
            editor.textContent = '';
            fireEvent.keyDown(editor, { key: 'Enter' });
    ...
    it('drains the next queued prompt on Enter when idle with a truly empty editor', async () => {
        const onDrain = vi.fn();
        const onSubmit = vi.fn();
        const { getByTestId } = render(_jsx(Harness, { onCancel: vi.fn(), onDrain: onDrain, onQueue: vi.fn(), onSubmit: onSubmit, queued: ['queued-1'] }));
        const editor = getByTestId('editor');
        await act(async () => {
            editor.textContent = '';
            fireEvent.keyDown(editor, { key: 'Enter' });
    ...
    it('keeps reconnect drafts editable but blocks Enter submit until the gateway returns', async () => {
        const onSubmit = vi.fn();
        const onDrain = vi.fn();
        const { getByTestId } = render(_jsx(Harness, { disabled: true, onCancel: vi.fn(), onDrain: onDrain, onQueue: vi.fn(), onSubmit: onSubmit, queued: ['queued-1'] }));
        const editor = getByTestId('editor');
        await act(async () => {
