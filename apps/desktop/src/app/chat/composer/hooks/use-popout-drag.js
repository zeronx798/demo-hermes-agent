...
import { POPOUT_ESTIMATED_HEIGHT, POPOUT_WIDTH_REM, readPopoutBounds, setComposerPopoutPosition } from '@/store/composer-popout';
...
function gestureTargetOk(target) {
...
function isFloatDragPlatform(target) {
    if (!(target instanceof Element)) {
        return false;
    }
...
const clampOffset = (value, max) => Math.min(Math.max(0, value), max);
/** Fixed-position composer uses bottom/right insets; keep the grab point under the pointer. */
function popoutPositionUnderPointer(clientX, clientY, grabX, grabY, boxWidth, boxHeight) {
    return {
 ...
 */
export function useComposerPopoutGestures({ composerRef, onDock, onPopOut, poppedOut, position }) {
    ...
    const beginFloatDrag = useCallback((state, clientX, clientY, next, size) => {
        clearTimer();
    ...
    const peelOffFromDock = useCallback((state, clientX, clientY) => {
