...
import { desktopSkinSlashCompletions, desktopSlashDescription, filterDesktopCommandsCatalog, isDesktopSlashExtensionCommand, isDesktopSlashSuggestion } from '@/lib/desktop-slash-commands';
    ...
    const fetcher = useCallback(async (query) => {
        ...
        if (sessionArg) {
            ...
            const matches = (needle
                ? $sessions
                    .get()
                    .filter(session => sessionTitle(session).toLowerCase().includes(needle) ||
                    (session.preview ?? '').toLowerCase().includes(needle) ||
                    session.id.toLowerCase().includes(needle))
                : $sessions.get()).slice(0, SESSION_INLINE_LIMIT);
        ...
        try {
            if (!query) {
                ...
                const items = sections.flatMap(section => section.pairs.map(([command, meta]) => ({
                    text: command,
                    display: command,
            ...
            const result = await gateway.request('complete.slash', {
                text
            });
