...
import { $composerAttachments, stashSessionDraft, takeSessionDraft } from '@/store/composer';
import { isBrowsingHistory } from '@/store/composer-input-history';
import { cloneAttachments, DRAFT_PERSIST_DEBOUNCE_MS } from '../composer-utils';
...
export function useComposerDraft({ activeQueueSessionKey, focusKey, inputDisabled, queueEditRef, sessionId }) {
    ...
    const stashAt = (scope, text = draftRef.current, attachments = $composerAttachments.get()) => stashSessionDraft(scope, text, attachments);
