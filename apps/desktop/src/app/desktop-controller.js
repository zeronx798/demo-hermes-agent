...
function sessionMatchesStoredId(session, id) {
...
function sessionMessagesSignature(messages) {
    let hash = 2166136261;
...
export function DesktopController() {
    ...
    const modelMenuContent = useMemo(() => gatewayState === 'open' ? (_jsx(ModelMenuPanel, { gateway: gatewayRef.current || undefined, onSelectModel: selectModel, requestGateway: requestGateway })) : null, [gatewayRef, gatewayState, requestGateway, selectModel]);
    ...
    const { archiveSession, branchCurrentSession, branchStoredSession, createBackendSessionForSend, openSettings, removeSession, resumeSession, selectSidebarItem, startFreshSessionDraft } = useSessionActions({
        activeSessionId,
        activeSessionIdRef,
        busyRef,
        creatingSessionRef,
    ...
    const { cancelRun, editMessage, handleThreadMessagesChange, reloadFromMessage, restoreToMessage, steerPrompt, submitText, transcribeVoiceAudio } = usePromptActions({
        activeSessionId,
        activeSessionIdRef,
    ...
    const reviewPane = (_jsx(Pane, { defaultOpen: true, 
        ...
        disabled: !chatOpen || !currentCwd.trim() || (!narrowViewport && !reviewOpen), forceCollapsed: narrowViewport, hoverReveal: true, id: REVIEW_PANE_ID, maxWidth: FILE_BROWSER_MAX_WIDTH, minWidth: FILE_BROWSER_MIN_WIDTH, 
