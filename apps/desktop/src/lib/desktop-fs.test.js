...
import { desktopDefaultCwd, desktopFileDiff, desktopGitRoot, readDesktopDir, readDesktopFileDataUrl, readDesktopFileText, selectDesktopPaths, setDesktopFsRemotePicker } from './desktop-fs';
...
describe('desktop filesystem facade', () => {
    ...
    it('targets the active profile backend so a remote profile never reads local disk', async () => {
        $connection.set({ mode: 'remote', profile: 'remote-docker' });
