const recency = (session) => session.last_active || session.started_at || 0;
/** Flat list with branch/fork sessions nested visually under their parent. */
export function flattenSessionsWithBranches(sessions) {
    ...
    const groupRecency = (session) => {
        const cached = groupRecencyMemo.get(session.id);
        if (cached !== undefined) {
            return cached;
        }
        groupRecencyMemo.set(session.id, recency(session)); // cycle guard
        const max = (childrenByParent.get(session.id) ?? []).reduce((acc, child) => Math.max(acc, groupRecency(child)), recency(session));
