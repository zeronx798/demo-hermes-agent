...
import { appendAssistantTextPart, appendReasoningPart, chatMessageText, preserveLocalAssistantErrors, renderMediaTags, toChatMessages, upsertToolPart } from './chat-messages';
describe('toChatMessages', () => {
    ...
    it('renders MEDIA tags as assistant attachment links', () => {
        ...
        expect(chatMessageText(message)).toBe("[Audio: tts_20260501_222725.mp3](#media:%2FUsers%2Fbrooklyn%2F.hermes%2Fcache%2Faudio%2Ftts_20260501_222725.mp3)\n\nhow's that sound?");
    });
    it('keeps the generated image on the tool row while preserving agent prose', () => {
        ...
        const toolPart = message.parts.find((part) => part.type === 'tool-call' && part.toolName === 'image_generate');
...
describe('renderMediaTags', () => {
    it('renders standalone and inline MEDIA tags as links', () => {
        ...
        expect(renderMediaTags('audio: MEDIA:/tmp/voice.mp3 done')).toBe('audio: [Audio: voice.mp3](#media:%2Ftmp%2Fvoice.mp3) done');
...
describe('interleaved reasoning/text coalescing', () => {
    it('keeps narration contiguous when reasoning interrupts mid-sentence', () => {
        ...
        let parts = appendAssistantTextPart([], 'Let me ');
        ...
        expect(parts[0].text).toBe('Let me verify the full file is correct:');
        expect(parts[1].text).toBe('checking the file...');
    });
    it('merges reasoning bursts that straddle a narration fragment', () => {
        let parts = appendReasoningPart([], 'first thought ');
        ...
        expect(parts[0].text).toBe('first thought second thought');
        expect(parts[1].text).toBe('Working on it.');
    });
    it('starts a fresh text part after a tool call (segment boundary)', () => {
        let parts = appendAssistantTextPart([], 'Let me check.');
        ...
        expect(parts[2].text).toBe('Now editing.');
    });
    it('does not merge reasoning across a tool call', () => {
        let parts = appendReasoningPart([], 'before tool');
        parts = upsertToolPart(parts, { name: 'read_file', tool_id: 'tc-1' }, 'running');
...
describe('upsertToolPart', () => {
    ...
    it('keeps live todo rows stable across sparse progress payloads', () => {
        ...
        const progressed = upsertToolPart(first, {
            name: 'todo',
            preview: 'updating plan',
            tool_id: 'todo-1'
        }, 'running');
        const [part] = progressed;
        const args = part && 'args' in part ? part.args : {};
    ...
    it('archives todo state on completion and accepts explicit empty clears', () => {
        ...
        const cleared = upsertToolPart(completed, {
            name: 'todo',
            todos: [],
            tool_id: 'todo-1'
        }, 'complete');
        const completedResult = completed[0] && 'result' in completed[0] ? completed[0].result : {};
        const clearedResult = cleared[0] && 'result' in cleared[0] ? cleared[0].result : {};
    ...
    it('keeps parallel same-name tools distinct without explicit ids', () => {
        ...
        const startedReykjavik = upsertToolPart(startedTokyo, {
            context: 'reykjavik weather',
            name: 'web_search'
        }, 'running');
        const completedTokyo = upsertToolPart(startedReykjavik, {
            context: 'tokyo weather',
            message: 'tokyo done',
            name: 'web_search',
            summary: 'Did 5 searches'
        }, 'complete');
        const completedBoth = upsertToolPart(completedTokyo, {
            context: 'reykjavik weather',
            message: 'reykjavik done',
            name: 'web_search',
            summary: 'Did 5 searches'
        }, 'complete');
        const webParts = completedBoth.filter((part) => part.type === 'tool-call' && part.toolName === 'web_search');
        const contexts = webParts.map(part => String(part.args?.context || ''));
        const summaries = webParts.map(part => {
            if (!('result' in part) || !part.result || typeof part.result !== 'object') {
                return '';
            }
            return String(part.result.summary || '');
        });
    ...
    it('does not append phantom same-name tool rows for id-less progress updates', () => {
        ...
        const startedB = upsertToolPart(startedA, {
            context: 'kathmandu weather today and tomorrow forecast',
            name: 'web_search'
        }, 'running');
        const progressed = upsertToolPart(startedB, {
            name: 'web_search'
        }, 'running');
        const webParts = progressed.filter((part) => part.type === 'tool-call' && part.toolName === 'web_search');
        expect(webParts).toHaveLength(2);
    });
    it('matches id-less live starts with later identified completions', () => {
        ...
        const webParts = completed.filter((part) => part.type === 'tool-call' && part.toolName === 'web_search');
    ...
    it('matches id-less live starts with later identified progress updates', () => {
        ...
        const progressed = upsertToolPart(started, {
            context: 'reykjavik tashkent uzbekistan weather today and tomorrow forecast',
            name: 'web_search',
    ...
    it('reconciles preview-first progress rows with later stable-id starts', () => {
        ...
        const progressB = upsertToolPart(progressA, {
            name: 'web_search',
            preview: 'reykjavik weather'
        }, 'running');
        const startedA = upsertToolPart(progressB, {
        ...
        const completedA = upsertToolPart(startedB, {
            name: 'web_search',
            summary: 'Did 5 searches',
            tool_id: 'search-tokyo'
        }, 'complete');
        const completedB = upsertToolPart(completedA, {
            name: 'web_search',
            summary: 'Did 5 searches',
            tool_id: 'search-reykjavik'
        }, 'complete');
        const webParts = completedB
            .filter((part) => part.type === 'tool-call' && part.toolName === 'web_search')
            .map(part => ({
            id: part.toolCallId,
            query: String(part.args?.query || ''),
            summary: part.result && typeof part.result === 'object'
                ? String(part.result.summary || '')
                : ''
        }));
