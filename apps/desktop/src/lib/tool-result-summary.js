...
const ERROR_MSG_KEYS = ['message', 'reason', 'detail'];
...
const norm = (v) => (typeof v === 'string' ? tryJson(v) : v);
const titleCase = (k) => k
    .split(/[_\-.]+/)
    .filter(Boolean)
    .map(capitalize)
...
function clipInline(value, max = 180) {
    const c = value.replace(/\s+/g, ' ').trim();
    return c.length > max ? `${c.slice(0, max - 1)}…` : c;
}
function clipBlock(value, maxChars = 1800, maxLines = 18) {
    const t = value.trim();
    if (!t) {
        return '';
    }
...
const isWrapperKey = (k) => WRAPPER_KEYS.includes(k);
const skipField = (k, v) => isWrapperKey(k) || ((k === 'success' || k === 'ok') && v === true);
function summarizeScalar(v) {
    if (typeof v === 'string') {
        return clipInline(v);
    }
...
function summarizeRecordInline(record, depth) {
    if (depth > 3) {
...
function summarizeListItem(item, depth) {
    const v = norm(item);
    if (typeof v === 'string') {
        return clipInline(v);
    }
...
function formatFieldValue(value, depth) {
    const v = norm(value);
...
function formatArraySummary(value, depth) {
    if (!value.length) {
        return '';
    }
    const max = 6;
...
function formatRecordSummary(record, depth) {
...
function formatSummaryValue(value, depth) {
    if (depth > 4) {
        return '';
    }
    const v = norm(value);
...
function hasErrorSignal(record) {
...
function findNestedError(value, depth, seen) {
    if (depth > 5) {
        return '';
    }
    const v = norm(value);
...
export function formatToolResultSummary(value) {
    return formatSummaryValue(unwrapPayload(value), 0) || formatSummaryValue(value, 0);
}
export function extractToolErrorMessage(value) {
