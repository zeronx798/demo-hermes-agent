...
import { attachmentDisplayText, coerceThinkingText, optimisticAttachmentRef, parseCommandDispatch, parseSlashCommand } from './chat-runtime';
...
describe('optimisticAttachmentRef', () => {
    ...
    it('ignores a non-data preview url and uses the path ref', () => {
        const ref = optimisticAttachmentRef(attachment({ kind: 'image', detail: '/tmp/shot.png', previewUrl: 'https://example.com/x.png' }));
        expect(ref).toBe('@image:/tmp/shot.png');
    });
    it('passes non-image attachments straight through to attachmentDisplayText', () => {
        expect(optimisticAttachmentRef(attachment({ kind: 'file', refText: '@file:src/a.ts', previewUrl: DATA_URL }))).toBe('@file:src/a.ts');
    });
    ...
    it('returns null for an undefined attachment instead of throwing', () => {
        expect(() => optimisticAttachmentRef(undefined)).not.toThrow();
...
describe('attachmentDisplayText', () => {
    it('returns null for undefined|null instead of reading .kind/.refText on a hole', () => {
        expect(() => attachmentDisplayText(undefined)).not.toThrow();
        expect(attachmentDisplayText(undefined)).toBeNull();
        expect(attachmentDisplayText(null)).toBeNull();
    });
...
describe('coerceThinkingText', () => {
    ...
    it('drops empty thinking rewrite placeholder text', () => {
        expect(coerceThinkingText("◉_◉ processing... I don't see any current rewritten thinking or next thinking to process. Could you provide the thinking content you'd like me to rewrite?")).toBe('');
    });
});
