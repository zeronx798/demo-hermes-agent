 ...
 */
function hasCloseFenceLine(body, marker) {
...
function scrubBacktickNoise(text) {
...
function normalizeVisibleProse(text) {
    return text
...
function extend(out, lines) {
    for (const line of lines) {
        out.push(line);
    }
}
function pushProseFence(out, indent, info, lines) {
    if (info) {
        out.push(`${indent}${info}`.trimEnd());
    }
    extend(out, lines);
}
function findClosingFence(lines, start, marker) {
...
function rewriteLatexBracketDelimiters(text) {
    return text
        .replace(LATEX_INLINE_RE, (_, body) => `$${body}$`)
        .replace(LATEX_DISPLAY_RE, (_, body) => `$$${body}$$`);
}
...
export function preprocessMarkdown(text) {
    ...
    return strippedEmptyFences
        ...
        const transformed = normalizeVisibleProse(stripPreviewTargets(rewriteLatexBracketDelimiters(escapeCurrencyDollars(part))));
