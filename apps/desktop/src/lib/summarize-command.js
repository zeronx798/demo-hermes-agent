...
const basename = (head) => head.split('/').pop() || head;
...
function splitCompoundCommand(input) {
    const segments = [];
    let buf = '';
    let quote = null;
    for (let i = 0; i < input.length; i += 1) {
        ...
        const op = input.startsWith('&&', i) || input.startsWith('||', i)
            ? input.slice(i, i + 2)
            : ch === ';' || ch === '\n'
                ? ch
                : '';
...
function stripPipeTail(segment) {
...
function isBoundaryEcho(segment) {
    const words = splitWords(segment);
 ...
 */
export function summarizeShellCommand(raw) {
