...
export function sanitizeLanguageTag(tag) {
...
export function codiconForLanguage(language) {
...
export function codiconForFilename(path) {
...
function filenameExtToken(path) {
...
const SHIKI_LANGUAGE_BY_EXTENSION = {
    astro: 'astro',
    bash: 'bash',
    c: 'c',
    cc: 'cpp',
...
export function shikiLanguageForFilename(path) {
...
export function isLikelyProseFence(info, body) {
    ...
    return ((signals.bulletLines >= 2 && signals.hasMarkdown && signals.codeSignals <= 2) ||
        (signals.proseLines >= 3 && signals.codeSignals === 0));
}
export function isLikelyProseCodeBlock(language, code) {
