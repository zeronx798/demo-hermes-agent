...
const DISPLAY_KEYS = ['host_image', 'image'];
const ECHO_KEYS = ['host_image', 'image', 'agent_visible_image'];
...
function stringFields(record, keys) {
    return keys.map(key => record[key]).filter((v) => typeof v === 'string' && v.trim().length > 0);
}
...
export function generatedImageFromResult(result) {
...
export function generatedImageEchoSources(parts) {
...
export function stripGeneratedImageEchoes(text, sources) {
...
export function dedupeGeneratedImageEchoesInParts(parts) {
