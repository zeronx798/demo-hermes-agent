...
export function resolveProfileColor(name, overrides) {
    const key = (name ?? '').trim();
    if (!key || key === 'default') {
        return null;
    }
...
export const PROFILE_SWATCHES = Array.from({ length: 12 }, (_, index) => `hsl(${index * 30} ${PROFILE_TAG_SATURATION}% ${PROFILE_TAG_LIGHTNESS}%)`);
