...
const oauthConn = { authMode: 'oauth', wsUrl: 'ws://host/api/ws?ticket=stale' };
const tokenConn = { authMode: 'token', wsUrl: 'ws://host/api/ws?token=abc' };
describe('resolveGatewayWsUrl', () => {
    describe('oauth mode', () => {
        ...
        it('throws a reauth error instead of falling back to the stale cached ticket', async () => {
            ...
            await expect(resolveGatewayWsUrl({ getGatewayWsUrl }, oauthConn)).rejects.toBeInstanceOf(GatewayReauthRequiredError);
        });
