...
const THINKING_STATUS_PREFIX_RE = /^\s*(?:(?:[^\s.]{1,16})\s+)?(?:processing|thinking|reasoning|analyzing|pondering|contemplating|musing|cogitating|ruminating|deliberating|mulling|reflecting|computing|synthesizing|formulating|brainstorming)\.\.\.\s*/i;
const EMPTY_THINKING_PLACEHOLDER_RE = /\b(?:current rewritten thinking|next thinking to process|provide the thinking content|don't see any .*thinking)\b/i;
export function createClientSessionState(storedSessionId = null, messages = []) {
    return {
        storedSessionId,
        messages,
        branch: '',
...
export function coerceGatewayText(value) {
 ...
 */
export function coerceThinkingText(value) {
...
export function contextPath(path, cwd) {
    if (!cwd) {
        return path;
    }
...
export function attachmentId(kind, value) {
    return `${kind}:${value}`;
}
export function pathLabel(path) {
    return path.split(/[\\/]/).filter(Boolean).pop() || path;
}
export function attachmentDisplayText(attachment) {
 ...
 */
export function optimisticAttachmentRef(attachment) {
    if (!attachment) {
        return null;
    }
...
export function personalityNamesFromConfig(config) {
...
export function normalizePersonalityValue(value) {
    const trimmed = normalize(value);
    return !trimmed || trimmed === 'default' || trimmed === 'none' ? '' : trimmed;
}
export function parseSlashCommand(command) {
...
export function parseCommandDispatch(raw) {
...
export function quickModelOptions(data, currentProvider, currentModel) {
    const seen = new Set();
    const options = [];
    ...
    const add = (provider, providerName, model) => {
...
export function toRuntimeMessage(message) {
    ...
    if (role === 'user') {
        return {
            id: message.id,
            role,
            content: message.parts.filter((part) => part.type === 'text'),
            attachments: [],
            createdAt,
...
export function createToolMergeCache() {
    return new WeakMap();
}
 ...
 */
export function coalesceToolOnlyAssistants(messages, cache) {
    const out = [];
    for (const message of messages) {
        const prev = out.at(-1);
        if (prev && prev.role === 'assistant' && !prev.pending && !prev.hidden && isToolOnlyAssistant(message)) {
            const cached = cache.get(message);
            const merged = cached && cached.prev === prev && cached.prevParts === prev.parts && cached.parts === message.parts
                ? cached.merged
