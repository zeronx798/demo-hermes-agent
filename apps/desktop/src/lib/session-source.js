...
export function isMessagingSource(source) {
    const id = normalizeSessionSource(source);
    return id != null && MESSAGING_SOURCE_IDS.has(id);
}
export function normalizeSessionSource(source) {
 ...
 */
export function handoffOriginSource(handoffState, handoffPlatform) {
...
export function sessionSourceLabel(source) {
    const id = normalizeSessionSource(source);
    if (!id) {
        return null;
    }
...
export function sessionSourceSearchTerms(source) {
    const id = normalizeSessionSource(source);
