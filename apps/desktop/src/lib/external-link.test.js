...
import { __resetLinkTitleCache, ExternalLink, fetchLinkTitle, hostPathLabel, isTitleFetchable, LinkifiedText, PrettyLink, urlSlugTitleLabel } from './external-link';
const desktopWindow = window;
const initialHermesDesktop = desktopWindow.hermesDesktop;
function installDesktopBridge(partial = {}) {
...
describe('external link helpers', () => {
    it('formats URL fallbacks as host + path', () => {
        expect(hostPathLabel('https://www.getyourguide.com/culebra-island-l145468/from-fajardo-full-day-cordillera-islands-catamaran-tour-t19894/')).toBe('getyourguide.com/culebra-island-l145468/from-fajardo-full-day-cordillera-islands-catamaran-tour-t19894');
    });
    it('derives readable title fallbacks from URL slugs', () => {
        expect(urlSlugTitleLabel('https://www.getyourguide.com/fajardo-l882/from-fajardo-icacos-island-full-day-catamaran-trip-t19891/')).toBe('From Fajardo Icacos Island Full Day Catamaran Trip');
    });
    ...
    it('deduplicates in-flight title fetches and caches results', async () => {
        ...
        installDesktopBridge({ fetchLinkTitle: bridge });
    ...
    it('opens links via the desktop bridge', () => {
        ...
        installDesktopBridge({ openExternal: openExternal });
        render(_jsx(ExternalLink, { href: "https://example.com/path/to/resource", children: "Example link" }));
    ...
    it('renders pretty links with fetched titles and no host suffix', async () => {
        ...
        render(_jsx(LinkifiedText, { text: `Read ${url}` }));
    ...
    it('shows host/path fallback when title is unavailable', () => {
        ...
        render(_jsx(PrettyLink, { href: url }));
        const link = screen.getByTitle(url);
    ...
    it('normalizes scheme-less links before opening', () => {
        installDesktopBridge();
        render(_jsx(LinkifiedText, { text: "Source expedia.com/things-to-do/puerto-rico-el-yunque-rainforest-adventure" }));
    ...
    it('explicitOnly skips bare filename/domain tokens and only links explicit URLs', () => {
        installDesktopBridge();
        render(_jsx(LinkifiedText, { explicitOnly: true, pretty: false, text: 'Report  https://paste.rs/abc\nagent.log  https://paste.rs/def\nerrors.log' }));
    ...
    it('without explicitOnly, bare filename tokens are still linkified (default behavior)', () => {
        installDesktopBridge();
        render(_jsx(LinkifiedText, { pretty: false, text: "open agent.log please" }));
