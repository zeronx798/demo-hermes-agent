...
function embeddedImageRemovalRange(text, dataStart, dataEnd) {
    let start = dataStart;
    let end = dataEnd;
...
export function embeddedImageUrls(text) {
    return extractEmbeddedImages(text).images;
}
export function textWithoutEmbeddedImages(text) {
