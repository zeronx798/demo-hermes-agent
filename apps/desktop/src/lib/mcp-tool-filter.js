...
// `tools/mcp_tool.py`.
const asNames = (value) => Array.isArray(value) ? value.filter((v) => typeof v === 'string') : undefined;
const toolsObject = (server) => {
    const tools = server?.tools;
    return tools && typeof tools === 'object' && !Array.isArray(tools) ? tools : {};
};
export function readToolsFilter(server) {
    const tools = toolsObject(server);
    return { exclude: asNames(tools.exclude), include: asNames(tools.include) };
}
export function isToolEnabled(server, name) {
...
export function toggleToolInServer(server, name) {
    const { exclude, include } = readToolsFilter(server);
...
export const countEnabledTools = (server, names) => names.filter(name => isToolEnabled(server, name)).length;
