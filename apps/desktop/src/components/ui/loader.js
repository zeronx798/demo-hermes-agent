...
export function Loader({ className, label = 'Loading', pathSteps = 240, role = 'status', strokeScale = 1, type = 'rose-curve', ...props }) {
    ...
    return (_jsx("div", { ...props, "aria-label": props['aria-label'] ?? label, className: cn('inline-grid size-10 place-items-center text-primary', className), role: role, children: _jsx("svg", { "aria-hidden": "true", className: "size-full overflow-visible", fill: "none", viewBox: "0 0 100 100", children: _jsxs("g", { ref: groupRef, children: [_jsx("path", { opacity: "0.1", ref: pathRef, stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: config.strokeWidth * strokeScale }), Array.from({ length: config.particleCount }, (_, index) => (_jsx("circle", { fill: "currentColor", ref: node => {
                            particleRefs.current[index] = node;
                        } }, `${type}-${index}`)))] }) }) }));
}
function baseCurve(name, options) {
    return {
...
function thinkingCurve(name, petalCount, options) {
    return {
        ...baseCurve(name, {
            ...options,
            rotate: true,
...
function roseCurve(name, k, options) {
    return {
        ...baseCurve(name, {
            ...options,
            rotate: true,
...
function spiralPetalCurve(name, spiralR, particleCount) {
    return {
        ...baseCurve(name, {
            durationMs: 4600,
            particleCount,
...
function cardioidCurve(name, options) {
    return {
        ...baseCurve(name, {
            durationMs: 6200,
            particleCount: options.particleCount,
...
function buildPath(config, detailScale, steps) {
    return Array.from({ length: steps + 1 }, (_, index) => {
        const point = config.point(index / steps, detailScale);
        return `${index === 0 ? 'M' : 'L'} ${point.x.toFixed(2)} ${point.y.toFixed(2)}`;
    }).join(' ');
}
function detailScaleFor(time, config, phaseOffset) {
...
function particleFor(config, index, progress, detailScale, strokeScale) {
...
function rotationFor(time, config, phaseOffset) {
    if (!config.rotate) {
        return 0;
    }
