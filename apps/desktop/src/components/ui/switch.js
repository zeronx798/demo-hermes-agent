...
const switchThumbVariants = cva('pointer-events-none block rounded-full bg-foreground shadow-[0_0.0625rem_0.1875rem_color-mix(in_srgb,var(--dt-background)_50%,transparent)] ring-0 transition-transform data-[state=unchecked]:translate-x-0 data-[state=checked]:bg-background', {
    variants: {
        size: {
...
function Switch({ className, size, ...props }) {
    return (_jsx(SwitchPrimitive.Root, { className: cn(switchVariants({ size }), className), "data-slot": "switch", ...props, children: _jsx(SwitchPrimitive.Thumb, { className: switchThumbVariants({ size }), "data-slot": "switch-thumb" }) }));
}
export { Switch };
