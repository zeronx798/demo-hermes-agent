...
function hintedRatio(aspectRatio) {
    return (ASPECT_HINTS[String(aspectRatio ?? '')
        .toLowerCase()
        .trim()] ?? ASPECT_HINTS.landscape);
}
function isInlineSrc(path) {
...
export const GeneratedImage = ({ aspectRatio, result }) => {
    const { t } = useI18n();
    ...
    if (failed && image) {
        return (_jsxs("a", { className: "mt-2 inline-block font-semibold text-foreground underline underline-offset-4 decoration-current/20 wrap-anywhere", href: "#", onClick: event => {
                event.preventDefault();
                void window.hermesDesktop?.openExternal(mediaExternalUrl(image));
            }, children: [copy.openImage, ": ", mediaName(image)] }));
    }
    return (_jsxs(_Fragment, { children: [_jsxs("span", { "aria-label": pending ? t.assistant.tool.renderingImage : undefined, "aria-live": pending ? 'polite' : undefined, className: "group/image relative block max-w-full overflow-hidden rounded-2xl transition-[width,height] duration-300 ease-out", "data-slot": "aui_generated-image", role: pending ? 'status' : undefined, style: {
                    aspectRatio: ratio,
                ...
                }, children: [!canvasGone && (_jsx("div", { className: cn('absolute inset-0 transition-opacity duration-500 ease-out', loaded && 'opacity-0'), onTransitionEnd: () => loaded && setCanvasGone(true), children: _jsx(DiffusionCanvas, {}) })), src && (_jsx("button", { className: "absolute inset-0 block size-full cursor-zoom-in", onClick: () => setLightboxOpen(true), title: copy.openImage, type: "button", children: _jsx("img", { alt: "Generated image", className: cn('absolute inset-0 size-full object-contain opacity-0 transition-opacity duration-500 ease-out', loaded && 'opacity-100'), draggable: false, onError: () => setFailed(true), onLoad: event => {
                                const { naturalHeight, naturalWidth } = event.currentTarget;
                                if (naturalWidth && naturalHeight) {
                                    setRatio(naturalWidth / naturalHeight);
                                }
                                setLoaded(true);
                            }, src: src }) })), loaded && src && (_jsx(ImageActionButton, { className: "group-hover/image:opacity-100", copy: copy, onClick: download, saving: saving }))] }), src && (_jsx(ImageLightbox, { alt: "Generated image", copy: copy, onClick: download, onOpenChange: setLightboxOpen, open: lightboxOpen, saving: saving, src: src }))] }));
};
