...
const readTheme = (probe) => Object.fromEntries(TOKENS.map(key => {
...
const drawAsciiDiffusion = (ctx, theme, width, height, time) => {
    ...
    for (let row = -1; row <= rows + 1; row += 1) {
        for (let col = -1; col <= cols + 1; col += 1) {
            ...
            const bloom = Math.exp(-(radius * radius) / 0.075) * 0.72 +
                Math.exp(-((radius - (0.28 + Math.sin(angle * 5 + time * 0.16) * 0.035)) ** 2) / 0.0028) * 0.8;
            const contour = Math.exp(-((Math.sin(angle * 3 + radius * 17 - time * 0.17) * 0.5 + 0.5 - radius) ** 2) / 0.016) * 0.38;
            ...
            const signal = clamp(staticNoise * (1 - denoise) +
                latent * denoise +
    ...
    const veil = ctx.createRadialGradient(width * centerX, height * centerY, 0, width * centerX, height * centerY, Math.min(width, height) * (0.35 + denoise * 0.3));
...
export const DiffusionCanvas = () => {
    const canvasRef = useRef(null);
    const sizeRef = useRef({ width: 0, height: 0 });
    const themeRef = useRef(FALLBACKS);
    ...
    return _jsx("canvas", { className: "absolute inset-0 h-full w-full", ref: canvasRef });
};
