...
function applyFormatJson(view, onError) {
...
function blockHighlight(range) {
    return EditorView.decorations.compute([], state => {
        const clamp = (pos) => Math.max(0, Math.min(pos, state.doc.length));
...
export function CodeEditor({ apiRef, className, disabled = false, formatJson = false, framed = false, filePath, highlight, initialValue, onCancel, onChange, onCursorChange, onFormatJsonError, onSave }) {
    ...
    useEffect(() => {
        viewRef.current?.dispatch({
            effects: highlightConf.current.reconfigure(highlightFrom !== undefined && highlightTo !== undefined
    ...
    if (!framed) {
        return _jsx("div", { className: cn('h-full min-h-0 overflow-hidden', className), ref: hostRef });
    }
    ...
    return (_jsx("div", { className: cn('flex h-full min-h-0 flex-col overflow-hidden rounded-md border border-(--ui-stroke-tertiary)', className), children: _jsx("div", { className: "min-h-0 flex-1 overflow-hidden p-2", ref: hostRef }) }));
}
