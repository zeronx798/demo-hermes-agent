...
export function chunkLines(lines, perChunk) {
...
export function chunkTextLines(text, perChunk) {
    return chunkLines(text.split('\n'), perChunk).map(chunk => ({
        ...chunk,
        text: chunk.lines.join('\n')
    }));
}
export function useFixedRowWindow({ overscanRows, rowPx, rowsPerChunk, totalRows }) {
    ...
    const compute = useCallback((node) => {
    ...
    const [win, setWin] = useState(() => compute(null));
    ...
    // re-renders nothing.
    const sync = useCallback((node = scrollerRef.current) => {
        if (!node) {
            return;
        }
        const next = compute(node);
        setWin(prev => prev.startChunk === next.startChunk &&
