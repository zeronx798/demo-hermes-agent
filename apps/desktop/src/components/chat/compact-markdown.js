...
function tagged(Tag) {
    const Component = (({ className, ...rest }) => {
        const Element = Tag;
        return _jsx(Element, { className: cn(TAG_CLASSES[Tag], className), ...rest });
    });
    Component.displayName = `Md.${Tag}`;
    return Component;
}
function MarkdownAnchor({ children, className, href, ...rest }) {
    if (!href || !/^https?:\/\//i.test(href)) {
        return (_jsx("a", { className: cn('font-medium underline underline-offset-4 decoration-current/20', className), href: href, ...rest, children: children }));
    }
    return (_jsxs(ExternalLink, { className: cn('decoration-current/20', className), href: href, showExternalIcon: false, children: [children, _jsx(ExternalLinkIcon, {})] }));
}
function MarkdownCode({ className, ...rest }) {
    return (_jsx("code", { className: cn('rounded bg-muted/80 px-1 py-px font-mono text-[0.86em] text-muted-foreground', className), ...rest }));
}
function MarkdownTable({ className, ...rest }) {
    return (_jsx("div", { className: "mb-2 max-w-full overflow-x-auto rounded-md border border-border/60 last:mb-0", children: _jsx("table", { className: cn('w-full border-collapse text-[0.72rem] [&_tr]:border-b [&_tr]:border-border/50 last:[&_tr]:border-0', className), ...rest }) }));
}
const COMPONENTS = {
    a: MarkdownAnchor,
...
export function CompactMarkdown({ className, text }) {
    return (_jsx("div", { className: cn('max-w-full text-xs leading-relaxed text-muted-foreground/90 wrap-anywhere', className), children: _jsx(Streamdown, { components: COMPONENTS, controls: false, mode: "static", parseIncompleteMarkdown: false, children: text }) }));
}
