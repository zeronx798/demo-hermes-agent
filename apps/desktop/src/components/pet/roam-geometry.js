...
export const groundTop = (ledge, petH) => ledge.y - petH + FEET_DROP_PX;
/**
 ...
 */
export const overlapsX = (from, to) => Math.min(from.right, to.right) > Math.max(from.left, to.left) + 2;
/**
 ...
 */
export function resolveLedge(ledges, x, y, petH) {
    const bottom = y + petH;
    let best = null;
...
function floorY(width, height, petH) {
...
export function snapshotLedges(petW, petH) {
    const width = vw();
    const height = vh();
    const ledges = [{ left: 0, right: Math.max(0, width - petW), y: floorY(width, height, petH) }];
 ...
 */
export function overlayLedge(petW) {
