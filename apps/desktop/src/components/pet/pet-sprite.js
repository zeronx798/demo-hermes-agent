...
import { $petState } from '@/store/pet';
 ...
 */
export function roamWalkRow(dir, stateRows) {
    if (dir === 0) {
        return { mirror: false };
    }
 ...
 */
function PetSpriteImpl({ info, zoom = 1, stateOverride, rowOverride }) {
    ...
    useEffect(() => {
        ...
        const resolveRow = (rowName) => {
            const row = rows.indexOf(rowName);
            const state = ROW_TO_STATE[rowName];
            const count = Math.max(1, framesByRow?.[rowName] ?? framesByState?.[rowName] ?? (state ? framesByState?.[state] : 0) ?? frames);
    ...
    return (_jsx("canvas", { "aria-label": info.displayName ? `${info.displayName} pet` : 'pet', height: drawH, ref: canvasRef, style: { height: drawH, width: drawW }, width: drawW }));
}
/**
