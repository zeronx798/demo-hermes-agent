...
import { GROUND_EPS, groundTop, overlapsX, overlayLedge, resolveLedge, snapshotLedges } from './roam-geometry';
 ...
 */
export function usePetRoam({ enabled, containerRef, isInteracting, petW, petH, loopMs, overlayOpen, commit }) {
    useEffect(() => {
        if (!enabled) {
            $petMotion.set(null);
            $petRoamDir.set(0);
        ...
        const restY = (ledge) => groundTop(ledge, petH);
        ...
        const cur = { x: rect.left, y: rect.top };
        let phase = 'pause';
        ...
        const signal = (pose, dir) => {
            $petMotion.set(pose);
            $petRoamDir.set(dir);
        };
        const beginPause = (now) => {
            phase = 'pause';
        ...
        const settleOn = (ledge, now) => {
            cur.y = restY(ledge);
            curLedge = ledge;
            applyDom();
            beginPause(now);
        };
        const beginVertical = (ledge) => {
            targetLedge = ledge;
