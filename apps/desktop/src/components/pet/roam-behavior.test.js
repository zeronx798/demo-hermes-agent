...
import { chooseMove, dwellMs, HOP_CHANCE, pickStrollTarget, REST_CHANCE } from './roam-behavior';
// Deterministic rng that replays a fixed sequence (last value sticks).
const seq = (...vals) => () => vals.shift() ?? vals[vals.length - 1] ?? 0;
const RANGE = { maxMs: 13000, meanMs: 4000, minMs: 1500 };
const ledge = (left, right, y = 0) => ({ left, right, y });
describe('dwellMs', () => {
...
describe('chooseMove', () => {
    ...
    it('loafs far more than it roams over a long run (the whole point)', () => {
        let state = 0.314159;
        const rng = () => (state = (state * 16807 + 0.5) % 1);
        let rests = 0;
        const N = 20000;
        for (let i = 0; i < N; i++) {
