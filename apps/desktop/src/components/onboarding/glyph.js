...
export function GlyphText({ text }) {
    return (_jsx(_Fragment, { children: Array.from(text, (ch, i) => GLYPH_SET.has(ch) ? (_jsx("span", { className: "text-[0.62em]", children: ch }, i)) : (ch)) }));
}
function useDecoded(text) {
...
export function DecodedLabel({ leaving, text }) {
    const decoded = useDecoded(text.toUpperCase());
    return (_jsxs("span", { className: cn('inline-flex items-center font-mono text-xs font-semibold uppercase tracking-[0.28em] tabular-nums text-primary transition duration-[360ms] ease-out', leaving ? 'translate-y-2 opacity-0 saturate-0' : 'translate-y-0 opacity-100 saturate-100'), children: [_jsx(GlyphText, { text: decoded }), _jsx("span", { "aria-hidden": "true", className: "dither ml-1.5 -mr-[0.875rem] inline-block size-2 shrink-0 -translate-y-px rounded-[1px] text-primary", style: { animation: 'ob-decode-cursor 1s step-end infinite' } }), _jsx("style", { children: '@keyframes ob-decode-cursor { 0%, 49% { opacity: 1 } 50%, 100% { opacity: 0 } }' })] }));
}
...
export function HackeryButton({ disabled, label, loading, onClick }) {
    return (_jsxs("button", { className: cn('group inline-flex items-center gap-2 rounded-md border border-(--stroke-nous) px-6 py-2.5', 'font-mono text-xs font-semibold uppercase text-primary', 'transition-all duration-150 hover:border-primary/60 hover:bg-primary/[0.06]', 'disabled:pointer-events-none disabled:opacity-50'), disabled: disabled, onClick: onClick, type: "button", children: [_jsx("span", { className: "text-primary/40 transition-colors group-hover:text-primary", children: "[" }), loading ? _jsx(Loader2, { className: "size-3 animate-spin" }) : null, _jsx("span", { className: "-mr-[0.25em] pl-[0.25em] tracking-[0.25em]", children: label }), _jsx("span", { className: "text-primary/40 transition-colors group-hover:text-primary", children: "]" })] }));
}
