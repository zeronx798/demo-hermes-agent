...
import { Children, isValidElement, useCallback, useContext, useEffect, useMemo, useRef, useState } from 'react';
import { cn } from '@/lib/utils';
import { $paneStates, ensurePaneRegistered, setPaneHeightOverride, setPaneWidthOverride } from '@/store/panes';
import { PaneShellContext } from './context';
...
const widthToCss = (value, fallback) => value === undefined ? fallback : typeof value === 'number' ? `${value}px` : value;
...
function isRole(child, role) {
    return isValidElement(child) && child.type?.__paneShellRole === role;
}
function collectPanes(children) {
    const left = [];
    const right = [];
    let mainCount = 0;
...
export function PaneShell({ children, className, style }) {
    ...
    return (_jsx(PaneShellContext.Provider, { value: { mainColumn: ctxValue.mainColumn, paneById: ctxValue.paneById }, children: _jsx("div", { className: cn('relative grid h-full min-h-0', className), "data-pane-shell": "", style: composedStyle, children: children }) }));
}
export function Pane({ children, className, defaultOpen = true, divider = false, disabled = false, hoverReveal = false, maxHeight, minHeight, overlayWidth: overlayWidthProp, id, maxWidth, minWidth, onOverlayActiveChange, resizable = false, width }) {
    ...
    const overlayWidth = overlayWidthProp !== undefined
    ...
    const startResize = useCallback((event, axis) => {
    ...
    if (overlayActive) {
        ...
        return (_jsxs("div", { className: cn('group/reveal pointer-events-none relative min-w-0', className), "data-forced": forced ? '' : undefined, "data-pane-hover-reveal": forced ? 'open' : 'closed', "data-pane-id": id, "data-pane-open": "false", "data-pane-side": side, ref: paneRef, style: { gridColumn: slot.gridColumn, gridRow: slot.gridRow }, children: [_jsx("div", { "aria-hidden": "true", className: "pointer-events-auto absolute inset-y-0 z-30 [-webkit-app-region:no-drag]", "data-pane-reveal-trigger": "", style: { [edge]: HOVER_REVEAL_EDGE_GUTTER, width: HOVER_REVEAL_TRIGGER_WIDTH } }), _jsx("div", { className: cn('pointer-events-none absolute inset-y-0 z-30 overflow-hidden transition-transform delay-0', offscreen, 'group-hover/reveal:pointer-events-auto group-hover/reveal:translate-x-0 group-hover/reveal:delay-[var(--reveal-enter-delay)] group-hover/reveal:shadow-[var(--reveal-shadow)]', 'group-data-[forced]/reveal:pointer-events-auto group-data-[forced]/reveal:translate-x-0 group-data-[forced]/reveal:delay-0 group-data-[forced]/reveal:shadow-[var(--reveal-shadow)]'), style: {
                        [edge]: 0,
                        width: overlayWidth,
                    ...
                    }, children: _jsx("div", { className: "flex h-full w-full flex-col", children: children }) }, edge)] }));
    }
    return (_jsxs("div", { "aria-hidden": !open, className: cn('relative min-h-0 min-w-0 overflow-hidden', !open && 'pointer-events-none', className), "data-pane-id": id, "data-pane-open": open ? 'true' : 'false', "data-pane-side": slot.side, ref: paneRef, style: { gridColumn: slot.gridColumn, gridRow: slot.gridRow }, children: [canResize && (_jsxs("div", { "aria-label": `Resize ${id}`, "aria-orientation": sash.orientation, className: cn('group absolute z-20 [-webkit-app-region:no-drag]', sash.bar, !isBottomRow && (slot.side === 'left' ? 'right-0 translate-x-1/2' : 'left-0 -translate-x-1/2')), onPointerDown: e => startResize(e, axis), role: "separator", tabIndex: 0, children: [divider && _jsx("span", { className: cn('absolute bg-(--ui-stroke-secondary)', sash.line) }), _jsx("span", { className: cn('absolute bg-(--ui-sash-hover-border) opacity-0 transition-opacity duration-100 group-hover:opacity-100 group-focus-visible:opacity-100', sash.hover) })] })), children] }));
}
;
Pane.__paneShellRole = 'pane';
export function PaneMain({ children, className }) {
    const ctx = useContext(PaneShellContext);
    if (!ctx) {
        if (import.meta.env.DEV) {
    ...
    return (_jsx("div", { className: cn('flex min-h-0 min-w-0 flex-col overflow-hidden', className), "data-pane-main": "true", style: { gridColumn: `${ctx.mainColumn} / ${ctx.mainColumn + 1}`, gridRow: '1 / -1' }, children: children }));
}
;
PaneMain.__paneShellRole = 'main';
