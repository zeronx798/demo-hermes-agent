...
const StatusRow = ({ children, label, className, ...rest }) => (_jsx("div", { "aria-label": label, "aria-live": "polite", className: cn('flex max-w-full items-center gap-2 self-start text-sm text-muted-foreground/70', className), role: "status", ...rest, children: children }));
// Fixed label while auto-compaction runs — decoupled from backend status text.
const COMPACTION_LABEL = 'Summarizing thread';
const CompactionHint = () => (_jsx("span", { className: "shimmer min-w-0 truncate text-muted-foreground/55", children: COMPACTION_LABEL }));
export const CenteredThreadSpinner = () => {
    const { t } = useI18n();
    return (_jsx("div", { "aria-label": t.assistant.thread.loadingSession, className: "pointer-events-none absolute inset-0 z-1 grid place-items-center", role: "status", children: _jsx(Loader, { "aria-hidden": "true", className: "size-12 text-midground/70", pathSteps: 220, role: "presentation", strokeScale: 0.72, type: "rose-curve" }) }));
};
export const ResponseLoadingIndicator = () => {
    const { t } = useI18n();
    const elapsed = useElapsedSeconds();
    const compacting = useStore($compactionActive);
    return (_jsxs(StatusRow, { "data-slot": "aui_response-loading", label: compacting ? COMPACTION_LABEL : t.assistant.thread.loadingResponse, children: [_jsx("span", { "aria-hidden": "true", className: "dither inline-block size-3 rounded-[2px] text-midground/80 animate-pulse" }), compacting && _jsx(CompactionHint, {}), _jsx(ActivityTimerText, { seconds: elapsed })] }));
};
...
// nothing is parked.
export const BackgroundResumeNotice = () => {
    const { t } = useI18n();
    ...
    return (_jsxs("div", { "aria-live": "polite", className: "flex max-w-[min(86%,44rem)] items-center gap-1.5 self-center px-2 py-0.5 text-[0.6875rem] leading-5 text-muted-foreground/55", "data-slot": "aui_background-resume", role: "status", children: [_jsx(Codicon, { className: "text-muted-foreground/55", name: "sync", size: "0.75rem" }), _jsx("span", { className: "shimmer min-w-0 truncate", children: label })] }));
};
...
export const StreamStallIndicator = () => {
    ...
    return (_jsxs(StatusRow, { className: "mt-1.5", "data-slot": "aui_stream-stall", label: compacting ? COMPACTION_LABEL : 'Hermes is thinking', children: [_jsx("span", { "aria-hidden": "true", className: "dither inline-block size-3 rounded-[2px] text-midground/80 animate-pulse" }), compacting && _jsx(CompactionHint, {}), _jsx(ActivityTimerText, { seconds: elapsed })] }));
};
