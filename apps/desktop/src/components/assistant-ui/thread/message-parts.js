...
const ImageGenerateTool = ({ args, result }) => {
    const aspectRatio = typeof args?.aspect_ratio === 'string' ? args.aspect_ratio : undefined;
    return (_jsx("div", { className: "mt-1.5", children: _jsx(GeneratedImage, { aspectRatio: aspectRatio, result: result }) }));
};
const ChainToolFallback = props => {
    // todo parts are hoisted to a dedicated panel above the message content.
    if (props.toolName === 'todo') {
        return null;
    }
    if (props.toolName === 'image_generate') {
        return _jsx(ImageGenerateTool, { ...props });
    }
    if (props.toolName === 'clarify') {
        return _jsx(ClarifyTool, { ...props });
    }
    return _jsx(ToolFallback, { ...props });
};
const ThinkingDisclosure = ({ children, messageRunning = false, pending = false, timerKey }) => {
    const { t } = useI18n();
    ...
    return (_jsxs("div", { className: "text-[length:var(--conversation-tool-font-size)] text-(--ui-text-tertiary)", "data-slot": "aui_thinking-disclosure", ref: enterRef, children: [_jsx(DisclosureRow, { onToggle: () => setUserOpen(!open), open: open, children: _jsxs("span", { className: "flex min-w-0 items-baseline gap-1.5", children: [_jsx("span", { className: cn('text-[length:var(--conversation-tool-font-size)] font-medium leading-(--conversation-line-height) text-(--ui-text-secondary)', pending && 'shimmer text-foreground/55'), children: t.assistant.thread.thinking }), pending && (_jsx(ActivityTimerText, { className: "text-[length:var(--conversation-caption-font-size)] tabular-nums text-(--ui-text-tertiary)", seconds: elapsed }))] }) }), open && (_jsx("div", { className: cn(
                ...
                'mt-0.5 w-full min-w-0 max-w-full overflow-hidden wrap-anywhere pb-1', isPreview && 'thinking-preview max-h-40'), ref: scrollRef, children: _jsx("div", { ref: contentRef, children: children }) }))] }));
};
...
const ReasoningAccordionGroup = ({ children, endIndex, startIndex }) => {
    const messageId = useAuiState(s => s.message.id);
    const messageRunning = useAuiState(s => s.message.status?.type === 'running');
    const pending = useAuiState(s => s.thread.isRunning &&
    ...
    const hasContent = useAuiState(s => s.message.parts
        .slice(Math.max(0, startIndex), endIndex + 1)
        .some(p => p?.type === 'reasoning' && typeof p.text === 'string' && p.text.trim().length > 0));
    if (!hasContent) {
        return null;
    }
    return (_jsx(ThinkingDisclosure, { messageRunning: messageRunning, pending: pending, timerKey: `reasoning:${messageId}`, children: children }));
};
const ReasoningTextPart = ({ text, status }) => {
    const displayText = text.trimStart();
    const messageRunning = useAuiState(s => s.message.status?.type === 'running');
    const isRunning = status?.type === 'running' || messageRunning;
    return (_jsx(MarkdownTextContent, { containerClassName: "text-xs leading-snug text-muted-foreground/85", containerProps: { 'data-slot': 'aui_reasoning-text' }, isRunning: isRunning, text: displayText }));
};
