...
import { focusComposerInput, markActiveComposer, onComposerFocusRequest, onComposerInsertRequest } from '@/app/chat/composer/focus';
import { useAtCompletions } from '@/app/chat/composer/hooks/use-at-completions';
import { useSlashCompletions } from '@/app/chat/composer/hooks/use-slash-completions';
import { dragHasAttachments, droppedFileInlineRefs, insertInlineRefsIntoEditor } from '@/app/chat/composer/inline-refs';
import { composerPlainText, placeCaretEnd, refChipElement, renderComposerContents, RICH_INPUT_SLOT } from '@/app/chat/composer/rich-editor';
import { detectTrigger, textBeforeCaret } from '@/app/chat/composer/text-utils';
...
import { StickyHumanMessageContainer, StopGlyph, USER_ACTION_ICON_BUTTON_CLASS, USER_ACTION_ICON_SIZE, USER_BUBBLE_BASE_CLASS } from '@/components/assistant-ui/thread/user-message';
...
export const UserEditComposer = ({ cwd, gateway, sessionId }) => {
    const { t } = useI18n();
    ...
    const [triggerPlacement, setTriggerPlacement] = useState('top');
    ...
    const syncDraftFromEditor = useCallback((editor) => {
    ...
    const replaceTriggerWithChip = useCallback((item) => {
        const editor = editorRef.current;
        if (!editor || !trigger) {
            return;
        }
    ...
    const insertRefStrings = useCallback((refs) => {
        const editor = editorRef.current;
    ...
    const insertDroppedRefs = useCallback((candidates) => insertRefStrings(droppedFileInlineRefs(candidates, cwd)), [cwd, insertRefStrings]);
    ...
    const uploadOsDropRefs = useCallback(async (osDrops) => {
        ...
        for (const candidate of osDrops) {
            ...
            const kind = candidate.file?.type.startsWith('image/') || isImagePath(candidate.file?.name || path) ? 'image' : 'file';
            try {
                const uploaded = await uploadComposerAttachment({ detail: path, id: attachmentId(kind, path), kind, label: pathLabel(path), path }, { remote, requestGateway, sessionId });
    ...
    return (_jsx(ComposerPrimitive.Root, { className: "contents", "data-slot": "aui_edit-composer-root", children: _jsx(StickyHumanMessageContainer, { children: _jsxs("div", { className: "composer-human-message-container human-execution-message-top relative flex w-full items-start rounded-md bg-(--ui-chat-surface-background)", onBlur: handleEditBlur, onDragEnter: handleDragEnter, onDragLeave: handleDragLeave, onDragOver: handleDragOver, onDrop: handleDrop, ref: rootRef, children: [trigger && (_jsx(ComposerTriggerPopover, { activeIndex: triggerActive, items: triggerItems, kind: trigger.kind, loading: triggerLoading, onHover: setTriggerActive, onPick: replaceTriggerWithChip, placement: triggerPlacement })), _jsxs("div", { className: cn(USER_BUBBLE_BASE_CLASS, 'ui-prompt-input__container relative border-(--ui-stroke-secondary) data-[expanded=true]:min-h-20', COMPOSER_DROP_FADE_CLASS, dragActive && COMPOSER_DROP_ACTIVE_CLASS), "data-expanded": expanded ? 'true' : undefined, children: [_jsx("div", { "aria-label": copy.editMessage, autoCapitalize: "off", autoCorrect: "off", className: cn('ui-prompt-input-editor__input max-h-48 w-full resize-none bg-transparent p-0 pr-7 text-[length:var(--conversation-text-font-size)] text-foreground/95 outline-none', 'empty:before:content-[attr(data-placeholder)] empty:before:text-muted-foreground/60', '**:data-ref-text:cursor-default', expanded ? 'min-h-16' : 'min-h-[1.25rem]'), contentEditable: true, "data-placeholder": copy.editMessage, "data-slot": RICH_INPUT_SLOT, onBlur: () => window.setTimeout(closeTrigger, 80), onDragOver: handleDragOver, onDrop: handleDrop, onFocus: () => markActiveComposer('edit'), onInput: handleInput, onKeyDown: handleKeyDown, onKeyUp: handleKeyUp, onMouseUp: refreshTrigger, onPaste: handlePaste, ref: editorRef, role: "textbox", spellCheck: false, suppressContentEditableWarning: true }), _jsx(ComposerPrimitive.Input, { asChild: true, className: "sr-only", submitMode: "ctrlEnter", tabIndex: -1, unstable_focusOnScrollToBottom: false, children: _jsx("textarea", { "aria-hidden": true, autoCapitalize: "off", autoComplete: "off", autoCorrect: "off", className: "sr-only", spellCheck: false, tabIndex: -1 }) }), staging && (_jsxs("span", { className: "pointer-events-none absolute bottom-2 left-2 inline-flex items-center gap-1 rounded-full bg-background/80 px-1.5 py-0.5 text-[0.62rem] text-muted-foreground backdrop-blur-[1px]", "data-slot": "aui_edit-staging", children: [_jsx(Loader2Icon, { className: "size-3 animate-spin" }), copy.attachingFile] })), _jsx("button", { "aria-label": copy.sendEdited, className: cn('absolute right-2 bottom-2 size-5', USER_ACTION_ICON_BUTTON_CLASS), disabled: !canSubmit || submitting || staging, onClick: () => {
                                    const editor = editorRef.current;
                                    if (editor) {
                                        submitEdit(editor);
                                    }
                                }, title: copy.sendEdited, type: "button", children: submitting ? StopGlyph : _jsx(Codicon, { name: "arrow-up", size: USER_ACTION_ICON_SIZE }) })] })] }) }) }));
};
