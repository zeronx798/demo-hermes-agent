...
const HERMES_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'];
...
export function directiveIconElement(type) {
...
const SLASH_CHIP_VARIANT = {
    command: 'bg-[color-mix(in_srgb,var(--ui-accent)_14%,transparent)] text-[color-mix(in_srgb,var(--ui-accent)_82%,var(--foreground))]',
    skill: 'bg-[color-mix(in_srgb,var(--ui-warm)_18%,transparent)] text-[color-mix(in_srgb,var(--ui-warm)_82%,var(--foreground))]',
    theme: 'bg-[color-mix(in_srgb,var(--ui-accent-secondary)_16%,transparent)] text-[color-mix(in_srgb,var(--ui-accent-secondary)_82%,var(--foreground))]'
};
export const SLASH_CHIP_BASE_CLASS = 'mx-0.5 inline-flex max-w-64 items-center gap-1 rounded px-1.5 py-0.5 align-middle text-[0.86em] font-medium leading-none';
export function slashChipClass(kind) {
    return `${SLASH_CHIP_BASE_CLASS} ${SLASH_CHIP_VARIANT[kind]}`;
}
export function slashIconElement(kind) {
    return iconElementFromPaths(SLASH_ICON_PATHS[kind]);
}
const DirectiveIcon = ({ type }) => (_jsx("svg", { className: "size-3 shrink-0 opacity-80", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, viewBox: "0 0 24 24", xmlns: "http://www.w3.org/2000/svg", children: iconPathsFor(type).map(d => (_jsx("path", { d: d }, d))) }));
...
export const DIRECTIVE_CHIP_CLASS = 'mx-0.5 inline-flex max-w-56 items-center gap-1 rounded px-1.5 py-0.5 align-middle text-[0.86em] font-normal leading-none bg-[color-mix(in_srgb,currentColor_8%,transparent)] text-muted-foreground';
/**
...
const HERMES_DIRECTIVE_RE = new RegExp('@(file|folder|url|image|tool|line|terminal|session):(' +
    '`[^`\\n]+`' +
    '|"[^"\\n]+"' +
    "|'[^'\\n]+'" +
    '|\\S+' +
...
export function formatRefValue(value) {
...
export const hermesDirectiveFormatter = {
    serialize(item) {
...
function safeEmbeddedImages(text) {
    try {
...
function safeDirectiveSegments(text) {
    try {
 ...
 */
export function DirectiveContent({ text }) {
    ...
    return (_jsxs("span", { className: "whitespace-pre-line", "data-slot": "aui_directive-text", children: [segments.map((segment, index) => segment.kind === 'text' ? (_jsx(Fragment, { children: segment.text }, `t-${index}`)) : segment.type === 'image' ? (_jsx(DirectiveImage, { id: segment.id, label: segment.label }, `img-${index}-${segment.id}`)) : (_jsx(DirectiveChip, { id: segment.id, label: segment.label, type: segment.type }, `m-${index}-${segment.id}`))), images.length > 0 && (_jsx("span", { className: "mt-2 flex flex-wrap gap-2", "data-slot": "aui_embedded-images", children: images.map((src, index) => (_jsx(ZoomableImage, { alt: "", className: "max-h-48 max-w-full rounded-lg border border-border/60 object-contain", draggable: false, slot: "aui_embedded-image", src: src }, `img-${index}`))) }))] }));
}
/** assistant-ui adapter: same renderer, exposed as a TextMessagePartComponent. */
export const DirectiveText = ({ text }) => (_jsx(DirectiveContent, { text: text ?? '' }));
...
const DirectiveImage = ({ id, label }) => {
    const isUrl = /^(?:https?|data):/i.test(id);
    const [src, setSrc] = useState(isUrl ? id : null);
    ...
    useEffect(() => {
        ...
        const load = window.hermesDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.hermesDesktop?.readFileDataUrl(id);
    ...
    if (failed) {
        return _jsx(DirectiveChip, { id: id, label: label, type: "image" });
    }
    if (!src) {
        return (_jsx("span", { "aria-hidden": true, className: "inline-block size-12 shrink-0 animate-pulse rounded-md bg-[color-mix(in_srgb,currentColor_8%,transparent)]" }));
    }
    return (_jsx(ZoomableImage, { alt: label, className: "max-h-32 max-w-48 rounded-md border border-border/40 object-contain", draggable: false, slot: "aui_directive-image", src: src }));
};
const DirectiveChip = ({ type, label, id }) => (_jsxs("span", { className: DIRECTIVE_CHIP_CLASS, "data-directive-id": id, "data-directive-type": type, "data-slot": "aui_directive-chip", title: id, children: [_jsx(DirectiveIcon, { type: type }), _jsx("span", { className: "truncate", children: label })] }));
