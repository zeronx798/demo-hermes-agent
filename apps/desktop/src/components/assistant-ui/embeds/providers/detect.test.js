...
describe('detectEmbed — other frame providers', () => {
    ...
    it('resolves Instagram posts and reels', () => {
        expect(frame('https://www.instagram.com/p/CabcDEF123/').embedUrl).toBe('https://www.instagram.com/p/CabcDEF123/embed');
    ...
    it('resolves Pinterest pins across locale hosts', () => {
        expect(frame('https://www.pinterest.com/pin/1234567890/').embedUrl).toBe('https://assets.pinterest.com/ext/embed.html?id=1234567890');
    ...
    it('resolves TikTok videos to the official player', () => {
        expect(frame('https://www.tiktok.com/@user/video/7212345678901234567').embedUrl).toBe('https://www.tiktok.com/player/v1/7212345678901234567');
    });
    it('resolves Spotify tracks, collections, and locale-prefixed urls', () => {
        expect(frame('https://open.spotify.com/track/4cOdK2wGLETKBW3PvgPWqT').embedUrl).toBe('https://open.spotify.com/embed/track/4cOdK2wGLETKBW3PvgPWqT');
        expect(frame('https://open.spotify.com/playlist/37i9dQZF1DXcBWIGoYBM5M').provider).toBe('spotify');
        expect(frame('https://open.spotify.com/intl-de/album/1DFixLWuPkv3KT3TnV35m3').id).toBe('spotify:album:1DFixLWuPkv3KT3TnV35m3');
...
describe('detectEmbed — Twitter/X', () => {
    it('resolves twitter.com and x.com status urls to a tweet descriptor', () => {
        for (const url of ['https://twitter.com/jack/status/20', 'https://x.com/jack/status/20']) {
            ...
            expect(descriptor.tweetId).toBe('20');
        }
    });
});
