...
import { buildToolView, clampForDisplay, countDiffLineStats, inlineDiffFromResult, MAX_TOOL_RENDER_CHARS } from './fallback-model';
...
describe('buildToolView terminal exit-code status', () => {
    const terminal = (result) => buildToolView(part({ result, toolName: 'terminal' }), '');
    ...
    it('keeps explicit error signals red even with output', () => {
        ...
        expect(buildToolView(part({ isError: true, result: { output: 'x' }, toolName: 'terminal' }), '').status).toBe('error');
    });
});
...
describe('buildToolView title actions', () => {
    it('marks the pending action separately from the rest of the title', () => {
        ...
        const web = buildToolView(part({ args: { url: 'https://example.com/docs' }, result: undefined, toolName: 'web_extract' }), '');
        const terminal = buildToolView(part({ args: { command: 'npm test -- --runInBand' }, result: undefined, toolName: 'terminal' }), '');
        const code = buildToolView(part({ args: { code: 'print("hello")' }, result: undefined, toolName: 'execute_code' }), '');
    ...
    it('uses the filename for completed read_file rows', () => {
        const view = buildToolView(part({ args: { path: './package.json' }, result: { content: '1|{"name":"demo"}' }, toolName: 'read_file' }), '');
