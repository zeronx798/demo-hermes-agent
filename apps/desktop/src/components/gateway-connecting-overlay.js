...
function scrambledTail(resolvedCount) {
    return Array.from(TAIL, (ch, i) => i < resolvedCount ? ch : SCRAMBLE_CHARS[(Math.random() * SCRAMBLE_CHARS.length) | 0]).join('');
}
export function GatewayConnectingOverlay() {
    ...
    return (_jsxs("div", { className: cn('fixed inset-0 z-[1200] grid place-items-center bg-(--ui-chat-surface-background) transition-opacity duration-500 ease-out', overlayHidden ? 'pointer-events-none opacity-0' : 'opacity-100'), children: [_jsx("style", { children: '@keyframes gco-cursor { 0%, 49% { opacity: 1 } 50%, 100% { opacity: 0 } }' }), _jsxs("span", { className: cn('inline-flex items-center pl-[0.4em] font-mono text-[0.64rem] font-semibold uppercase tracking-[0.4em] tabular-nums text-(--theme-primary) transition duration-300 ease-out', leaving ? 'translate-y-2 opacity-0 saturate-0' : 'translate-y-0 opacity-100 saturate-100'), children: [PREFIX, tail, _jsx("span", { "aria-hidden": "true", className: "dither ml-0.5 inline-block size-2 shrink-0 -translate-y-px rounded-[1px]", style: { animation: 'gco-cursor 1s step-end infinite' } })] })] }));
}
